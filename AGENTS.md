...
- **Forwarded operator quit keys own the keepalive prompt** — when stdin-forwarded `Ctrl+D`, a terminating stdin operator `Ctrl+C`, or stdin operator `Ctrl+Z` reaches a Codex supervisor, `start.rs` must return to the restart-or-quit prompt even if the run already committed a document cycle. Codex `Ctrl+C` and `Ctrl+Z` must be intercepted by the supervisor, logged as `codex_operator_control`, converted to child signals, and never forwarded as literal bytes into the child composer/transcript. Keep that rule aligned in `start.rs`, `README.md`, `SPEC.md`, and the supervisor/Codex specs.
...
- **CI checks are passive turn evidence, not a blocking gate** — after local verification and push, check GitHub Actions with one-shot commands such as `gh run list --workflow CI --limit 1` or `gh run view <run-id> --json status,conclusion`. Record `queued`, `in_progress`, `success`, or failure details in the closeout, but never wait on CI completion during an agent-doc turn and never run `gh run watch` unless the user explicitly asks. If CI is still queued or running, continue the closeout from local verification evidence so queue draining is not stalled by remote runner latency.
...
For local verification, run `make check` before committing. After local changes, check the latest GitHub Actions CI run once with `gh run list --workflow CI --limit 1`; record queued/in-progress/success/failure evidence, fix failures that have already started and are actionable, and do not block the turn waiting for CI completion unless the user explicitly asks.
