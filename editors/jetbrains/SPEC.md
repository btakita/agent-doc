...
- Active/busy pane and protected prompt-input clear refusals must show a typed warning with the relevant retry or interrupt actions, `Show status`, and `Copy details` instead of the generic command-failed text. For an active `agent-doc` pane, the warning must state that the session is still running, name the pane/current command, preserve the latest pane output, and offer refresh/retry plus explicit interrupt-clear recovery.
