...
- **Codex keepalive EOF/Ctrl-D can no longer close the tmux pane on the normal path.** The remaining `monsterrodholders.md` pane-drop repro was simpler than the earlier committed-turn successor case: a fresh Codex run could still log `restart_eval ... ctrl_d=true ... action=prompt_user`, then immediately record `user_quit_after_ctrl_d` or `user_quit_after_eof` and end the session without an intentional operator choice in the live pane. `start.rs` now treats forwarded stdin EOF/Ctrl-D as a fail-closed restart signal for Codex keepalive flows instead of surfacing the quit prompt at all, and the remaining resume-failure prompt now treats prompt-time EOF as `restart fresh` rather than `quit`. Added start-level regression coverage for the stricter restart strategy. This closes the latest “never kill” regression against `src/boost-client/tasks/monsterrodholders.md`.
