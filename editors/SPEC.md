...
- **Clear Session Context:** Run `agent-doc session clear <relative-path>` so the authoritative session receives the harness-native clear command instead of the plugin pasting `/clear` directly into tmux. The next Run action must still dispatch the bare reopen into that same session. Plugins must not block clear on plugin-local response-status or busy flags. The binary owns live pane evidence for status and diagnostics: stale busy projection is reconciled only when `agent-doc session status` / `session clear` observes direct `alive-idle` evidence with `prompt_ready=true`, while direct `alive-busy` evidence fails closed. If clear is refused because the protected pane is `alive-busy`, plugins must surface a running-session warning with `Refresh and retry`, `Interrupt and clear`, `Show status`, and `Copy details` actions instead of showing the raw command failure. `Refresh and retry` first runs `agent-doc session status <relative-path>` and only retries clear when the returned `live_pane:` line is `state=alive-idle` with `prompt_ready=true`. `Interrupt and clear` must require explicit confirmation and then run `agent-doc session interrupt-clear <relative-path>`; it is the only plugin action that intentionally interrupts a busy turn.
