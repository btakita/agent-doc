...
- **Accretion advisory** — `session_accretion` still summarizes exchange growth, closeout churn, and restart-heavy reopen signals so prompts can choose bounded recent-context packs or suggest compaction/restart when useful. General exchange-size accretion remains advisory, while repeated no-op closeout churn becomes a `compact` handoff in `agent-doc plan` so the next pass compacts or restarts instead of appending another low-signal `### Re:` section.
