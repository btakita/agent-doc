...
- **backlog-required closeout gate:** if the pre-response prompt contract requested backlog capture directly or through prompt-preset expansion (for example `#code-review` => `#follow-up-backlog`), `finalize` must fail before commit unless the cycle recorded backlog mutations or the response explicitly says there were no actionable follow-up items to capture. This gate is contract-driven, not heuristic-only, so findings-only review prose cannot silently commit against an unchanged backlog even when `pending_capture_guard` is merely `warn`.
- **pre-commit pending-done gate** (strict mode only): after the write succeeds and before the commit step, `finalize` also compares the active response capture against still-open tracked-work ids from `agent:backlog` / `agent:pending` and `agent:icebox`. When the response clearly completes an existing `#id` item but the cycle recorded no matching `--pending-done <id>`, `finalize` exits non-zero before committing. Session documents default this guard to `strict`; frontmatter or project config may downgrade it to `warn` or `off`. The `<!-- no-pending-done-guard -->` marker suppresses the gate for intentional exceptions.
