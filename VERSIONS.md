...
- **Managed Codex supervisors now keep routine stderr out of the foreground TUI (`#codexstderrtui`).** Codex is now classified with OpenCode as a TUI harness for `agent-doc start`, so supervisor `eprintln!` diagnostics are redirected to `.agent-doc/logs/supervisor-stderr.log` instead of printing over the Codex screen after `/clear`, restart, stale-busy reconcile, or stale-binary hot-reload. Claude remains attached to supervisor stderr for its existing operator prompt contract. Coverage: `is_tui_harness`.
