...
- On a real markdown selection event, JetBrains should also start a best-effort immediate `agent-doc focus <file>` for the event's selected document before debounce or sync-lock acquisition. Default focus is the fast no-promotion handoff; missing/dead pane failures and over-budget focus subprocesses are logged and ignored; the guarded `sync --no-autostart` reconciliation remains the authoritative background layout repair.
...
- `Run Agent Doc` waits for the active markdown document's typing indicator to go idle before saving and dispatching with `agent-doc route --dispatch-only --plain-trigger --wait-for-ready 120`. The binary route command also owns a default quiescence gate before any route-side frontmatter write, duplicate prompt cleanup, or pane submit, and the explicit startup-readiness wait gives editor-initiated reroutes a longer window for slow supervisor startup before surfacing an error. Even after `Clear Session Context`, this action still sends the plain `agent-doc <FILE>` reopen into the live session instead of restarting Codex.
- Every `Run Agent Doc` click writes a durable attempt ledger entry for click receipt, typing-idle wait, document save, route command construction/start, retry/supersede, and terminal route outcome. The ledger may persist route command text and route output summaries, but it must not persist raw document prompt text; prompt/trigger proof is represented by byte counts and hashes in binary/controller diagnostics.
- Repeating `Run Agent Doc` while an older plugin-spawned route process is still alive must not cancel the older process. The first route process owns the submit/proof window; later clicks for the same document are deduped visibly and recorded in the durable attempt ledger so they cannot repeatedly kill the process that would have sent `Enter`.
- `Run Agent Doc` and `Clear Session Context` are serialized per document in the JetBrains action layer before either command reaches the binary. Repeated Run clicks still dedupe behind the first alive route process, but `Clear Session Context` preempts a still-dispatching Run by canceling the plugin-spawned route process and then running the normal binary-owned `agent-doc session clear <relative-path>` path. If Run is clicked while a normal clear command is already running, the latest Run intent is queued and starts only after the clear completes synchronously. A clear accepted for deferred delivery by the binary does not release the queued Run immediately.
- If route fails only because the authoritative actor is still in its startup window, `Run Agent Doc` retries the same dispatch up to three times with bounded backoff before surfacing the final route failure. Dispatch-only `latest run is still booting ... (timed_out)` failures are not retried because the binary already spent the ready-wait window; the plugin surfaces the persisted route diagnostic immediately. Active-turn blockers get a still-running notification, and protected-input blockers such as shell history search are not retried. Later `Run Agent Doc` clicks are deduped while that retry loop is alive; after it exits, a new click may start a new route.
- If route succeeds by queueing a prompt behind an already-busy authoritative actor instead of injecting a duplicate trigger, `Run Agent Doc` must surface a visible queued/still-running warning rather than treating the route process as silent success. Repeating `Run Agent Doc` after editing that same prompt must replace the sole live route-owned `agent:queue` prompt instead of leaving stale wording queued behind the active turn. The plugin accepts the new `active agent:queue` route diagnostic and the older `agent:queue auto` wording for compatibility.
...
- JetBrains must not apply a socket or file-watch patch while IntelliJ has a pending File Cache Conflict for that document. The plugin records the patch as conflict-deferred, retries after the dialog resolves, applies normally after the user chooses to load filesystem changes, and treats a still-unsaved memory/disk divergence as Cancel: socket IPC returns failure/already-applied as appropriate and file-watch IPC leaves the patch file in place without mutating the document. The conflict path also reschedules agent-doc visual highlighting while the dialog is pending, after Cancel, after the accepted/reloaded document is visible, and after a deferred patch is applied, so File Cache Conflict resolution cannot strand markdown syntax/highlighter ranges in the pre-conflict state. The binary-side follow-up contract for a Cancel-shaped closeout is: if the response already reached the working tree/snapshot but not `HEAD`, the next `agent-doc preflight` auto-commits the missing boundary rather than surfacing the older manual `write --commit` recovery requirement.
...
- `[layout-sync]` (`EditorTabSyncListener`) — `selectionChanged`, immediate `focus`, the full `exec:` command (including the `--col` editor-layout args derived from `[layout-detect]`), the `result:` line, plus debounce/guard/deferred/timeout/generation diagnostics.
...
Run, Fix Document, Claim, Compact Exchange, Sync Layout, Show Session Status, Restart Supervisor Process, Clear Session Context, Interrupt and Clear Session Context, and Copy Session Diagnostics are available in:
- Tools menu
