...
- When `Run Agent Doc` is invoked with a non-empty editor selection, JetBrains must capture that selection synchronously before scheduling background work and send it verbatim as `selected_text` with a stable per-attempt `steering_id`. This selected-text branch bypasses layout collection and ordinary dispatch-only routing, accepts only a typed Project Controller steering acknowledgement, and treats a duplicate acknowledgement as successful retry deduplication. The active harness turn continues; the plugin must not submit a new `agent-doc <FILE>` trigger. An invocation without a selection keeps the normal busy-pane no-new-trigger refusal.
