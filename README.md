...
- **Agent orchestration** — harness-specific routing for Claude, Codex, and OpenCode, prompt contracts, model attribution, direct-run dispatch, and sequential/parallel/DAG orchestration.
...
- **tsift graph-backed planning** — when a materialized `.tsift/graph.db` exists, `plan` and `orchestrate` attach `graph-db evidence` handles plus `conflict-matrix` ownership summaries to queued `do #id` work, fail closed on stale graph freshness or missing target ids, and pass bounded `<tsift_graph_evidence>` context to child/job prompts without writing that context into the session document
