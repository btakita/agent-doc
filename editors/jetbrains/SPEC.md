...
- If the retained focus projection reports `actor_pane_not_visible`, the listener generation-fences the receipt against the still-active project window and republishes the complete current editor surface with forced reconciliation. The full surface remains the structural authority; other focus failures do not trigger layout repair.
