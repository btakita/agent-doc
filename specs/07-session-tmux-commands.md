...
- Editor `Run Agent Doc` callers may retry the route command when the dispatch-only ready probe reports a transient latest-run booting shape, including `timed_out` or `active codex turn`. Interactive shell substates, queued drafts, hook review prompts, clean-exit restart prompts, and other protected-input blockers are terminal route refusals and must not be hidden behind editor retries.
...
- The supervisor idle-queue watch owns the later drain payload for that queued dispatch. Claude/OpenCode drains may inject the normal harness reopen, but Codex owner panes must receive an in-pane queue-continuation prompt rather than a recursive `agent-doc <FILE>` self-invocation.
...
- On a cross-session **Reject**, `claim` emits a stable machine-readable marker line to stderr **before** the human bail text so editor plugins (JetBrains "Claim for Tmux Pane", VS Code claim command) can branch on it and render a choice dialog (Force claim / Switch project session / Cancel) instead of surfacing the raw exit-1 message. The marker shape is `[claim] cross-session-reject pane_id=<id> pane_session=<session> configured=<session>` (`CROSS_SESSION_REJECT_MARKER` + stable `pane_id`/`pane_session`/`configured` field order, formatted by `cross_session_reject_marker`). The human bail message (`pane <id> is in tmux session '<session>' but project session is '<session>'; switch to the configured session or pass --force`) is preserved unchanged for terminal use. The plugin choice dialog itself is gated on live-IntelliJ/VS Code verification.
