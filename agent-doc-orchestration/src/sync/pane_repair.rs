...
pub(crate) struct SyncProofCache {
    pub(crate) actor_records: RefCell<HashMap<(PathBuf, String), Option<crate::session_actor::ActorRecord>>>,
...
pub(crate) fn missing_pane_closeout_block_reason(file: &Path, phase: &str, error: Option<&str>) -> String {
    if let Ok(Some(message)) = crate::session_check::detect_uncommitted_closeout_drift(file) {
        return message;
    }
