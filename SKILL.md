...
- **Preflight already ran — do not re-run it (`#preflightinbinary`)** — when the turn opens with the `[agent-doc] cycle contract (preflight already ran in the binary; ...)` marker, the binary ran the pipeline on the trigger prompt and the contract is in your context. Read it; do not shell back to `agent-doc preflight <FILE>`. Run preflight yourself only when that marker is absent (harness without the hook, mid-turn re-check, recovery).
