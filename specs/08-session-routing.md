...
or retained component write. After that boundary and the target/startup safety
checks, route sends the normalized bare trigger plus one submit key exactly once
and returns when tmux accepts that transport operation. It does not wait for
pane-capture acceptance or dispatch-start proof, and it sends no additional
submit keys. This rule supersedes the older busy-actor focus-only/refusal
behavior for a plain trigger; prompt-aware routes retain their queueing and
