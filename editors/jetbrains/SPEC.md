...
- A remote CRDT delivery is acknowledged only after its IntelliJ `Document` is cleanly saved. The save is permitted when a raw disk read still equals either the guarded editor baseline or the converged target; novel external disk text rejects the delivery without mutating or overwriting it. This keeps non-operator replica churn from leaving an unsaved buffer that later triggers IntelliJ's File Cache Conflict resolver while preserving fail-closed behavior for real external edits.
