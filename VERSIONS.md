...
- **Tmux submit now sends literal text and Enter as separate calls (`#tmuxenter`).** The supervisor, idle queue, and direct-pane submit paths no longer rely on a compound `send-keys -l <text> ; send-keys Enter` command, so a post-restart `/clear` or `agent-doc <FILE>` draft cannot hide a missed named Enter behind a successful text write. Route now also preserves redacted pane-output snapshots under `.agent-doc/logs/route-submit/` when an existing draft, missed submit, or accepted-without-proof dispatch needs forensic evidence. Coverage: `submit_profiles_keep_harness_submit_policy_in_one_place`, `pending_payload_enter_resubmit_is_scoped_and_one_shot`, `route_pane_snapshot_preserves_redacted_terminal_capture`.
...
- **JB `Run Agent Doc` and `Clear Session Context` on Codex now use a real tmux Enter submit (`#jbcodexsubmit`).** The shared `sessions::send_submitted_text_for_harness` live-pane path now sends Codex command text with `tmux send-keys -l <text>` and then a named `Enter` key instead of merging the text with a trailing carriage-return byte. That fixes the operator-observed `/clear` and `agent-doc <FILE>` drafts that reached the Codex composer but did not submit, while OpenCode stays on Kitty Return and other harnesses keep the CR path. Diagnostics now report `tmux_text_enter_key` / `routed_trigger_enter_key`; the existing one-shot resubmit remains as bounded recovery if an older draft is already visible. Coverage: `codex_submit_mode_uses_literal_text_plus_real_enter_key`, `routed_trigger_submit_diagnostic_names_codex_real_enter_key`, and `simworld_jb_run_and_clear_share_codex_named_enter_submit_contract`.
