...
**Automatic pruning:** `resync::prune()` (step 1 only — no issue detection or fixing) runs automatically before `route`, `sync`, and `claim` operations. Uses bulk metadata fetching (2 subprocess calls: `list-windows -a` + `list-panes -a`) instead of per-pane queries. Stranded panes (no valid return target) are deregistered on first failure to prevent repeated expensive lookups. **Stash pane safety:** unregistered agent processes (`agent-doc`, `claude`, `node`) in stash windows are purged only when they are truly orphaned: if the pane still proves ownership of a registered document or still hosts a live supervisor socket, prune must preserve it. The scoped `fix <FILE>` duplicate-owner recovery path may still kill redundant unregistered stash panes after it has already re-bound the document to a unique provable winner.
...
2. If snapshot exists:
   ...
   f. Historical lower bound: if `HEAD` already contains a safe historical agent-owned response growth beyond the stale snapshot, and the current working tree differs from that committed `HEAD` only by a new user follow-up in `exchange`, `commit` must repair the snapshot up to `HEAD` before the no-op `HEAD` comparison. This repair is exchange-only: it may adopt already-committed `exchange` prompt+response growth, but it must fail closed if the historical bypass also mutated typed components such as `status`, backlog, or pending. This prevents stale snapshots from staging an older blob and rewinding an already-committed response.
...
**Pre-sync file resolution:** Before the layout algorithm runs, sync parses file paths from `--col` args and resolves each file. Files without a session UUID in frontmatter are treated as **unmanaged** and skipped (no auto-initialization of frontmatter). Only `agent-doc claim` adds session UUIDs. Files with session UUIDs are always treated as **registered**, even if the registry entry was pruned (dead pane). This enables the declarative layout flow: navigating to a file in a split creates a tmux pane regardless of registry state. For managed files whose registered pane is in a stash window, sync **rescues** the pane back to the agent-doc window via guarded `join-pane`, choosing the left or right edge from the requested column so the recovered pane becomes visible without swapping another live pane back into stash. If the registered pane cannot be found anywhere and sync is about to replace it, sync first records a synthetic `supervisor_exit code=missing_pane` event in the owning session log and attempts the narrow stale-`preflight_started` repair path so abandoned cycles do not stay wedged open behind the replacement start. Only after that recovery step, or if no alive pane exists at all, does sync auto-start a fresh Claude session (via `route::auto_start()`).
...
Nested lists are also supported inside backlog and icebox items. The canonical tracked item is the flush-left parent entry (`- ...` or `1. ...`); any indented continuation lines that follow it belong to that same item and must move/reap/edit with the parent block rather than being parsed as standalone tracked work.
...
On the next route invocation, if the registered pane matches a persisted startup-miss marker, route first re-runs the live-owner proof. If that same pane still proves live ownership of the document, the marker is no longer cleared just because ownership matches: route may clear it only when the session log proves a newer open `session_start` for that pane after the recorded miss. If the same pane still owns the document but the session log only shows a closed/timeout restart loop with no newer start after the miss, route must deregister it and auto-start fresh instead of repeatedly reusing and re-clearing the same broken pane. If the marked pane is still alive but the session log for its latest `session_start` never recorded a later child exit or `session_end`, and the supervisor socket is missing/unreachable, route must fail closed and report that stranded-session provenance instead of auto-starting a replacement pane on top of it. Only when the marked pane no longer proves live ownership and the recorded session already closed (or there is no stranded-session evidence) does route deregister it, clear the marker, and auto-start fresh. This prevents startup-miss reruns from masquerading as a generic tmux-pane crash.
...
Before lazy-claim or auto-start, route also scans tmux for an already-running pane whose process tree is executing the same document and re-registers that pane when found. This prevents stale-registry drift from spawning a parallel fallback session for a document that is already live somewhere else. The same ownership proof now applies to an *alive* registered pane: route first tries the tmux process-tree match on the document path, accepting equivalent pane-relative start-path forms for the same file (for example `tasks/docs.md` in a submodule pane versus `src/session-share/tasks/docs.md` in the root registry), then falls back to the per-session supervisor socket by asking it for the live child PID and mapping that PID back to the owning tmux pane. If another pane does prove ownership, route re-registers there first and dispatches to the recovered live owner. If neither the tmux process tree nor the supervisor PID can prove that the registered pane still owns the file, route deregisters that stale binding and continues with lazy-claim / auto-start recovery instead of dispatching into the ambiguous pane. Any later `send-keys` dispatch must still re-check the registry entry for the chosen pane and refuse the send if that pane is currently registered for a different file.
