...
- **Manual Sync Tmux Layout repairs window order** — keep `sync.rs`, `README.md`, `SPEC.md`, editor specs, and the session/tmux command spec aligned on the full `agent-doc sync` repair invariant: windowless full sync must finish with `0:agent-doc`, `1:stash`, and adjacent overflow `N:stash` windows, renaming `stash-*` aliases back to `stash`. Passive `sync --no-autostart` remains non-destructive and must not run this repair step.
...
- **Tmux CI review is part of test-bearing turns** — whenever a turn runs or changes tests, review the latest GitHub Actions CI run for the tmux leg (`make tmux-ci`). If CI reports tmux failures, reproduce locally with `make tmux-ci`, fix the issue, and add or update deterministic SimWorld coverage for the failure class when that behavior can be modeled without live tmux. Keep `SKILL.md`, `SPEC.md`, `README.md`, and installed harness surfaces aligned on this rule.
