...
- File-scoped `agent-doc session clear <FILE>` must not run the starting-actor readiness guard. Clear is an explicit operator context-reset request; stale startup projection, `alive-busy` pane classification, and post-commit user edits are not enough to block the `/clear` submit boundary when no protected prompt-input state is visible.
