...
- IPC sidecar verification fallbacks must re-apply `normalize_prefix_lines` to `content_ours` before saving that fallback as the snapshot and must repair the working tree to the same normalized fallback before returning success; a bare prompt-target line in fallback content or plugin-written disk content must not become the next committed closeout baseline.
