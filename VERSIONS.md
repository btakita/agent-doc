...
- **JetBrains/Codex Run Agent Doc now trusts the shared Enter submit path and stops threshold-clear churn (`#jbsimpleroute`).** Dispatch-only Codex reroutes no longer fail the IDE action after tmux accepts the shared text+`Enter` submit merely because hook dispatch-start proof did not arrive inside the proof window; they complete as `proof=accepted proof_scope=accepted_only`, matching the simpler cross-harness delivery contract. The supervisor idle-queue watch also no longer sends transcript-threshold `/clear` commands for ordinary queue heads, even when `agent_doc_queue_context_reset` is enabled; only explicit operator clears and explicit `[clean-session]` heads can make the supervisor clear. Coverage: `dispatch_only_codex_with_visible_hooks_accepts_enter_delivery`, `dispatch_only_proof_policy_accepts_enter_delivery_for_all_harnesses`, `clean_session_head_forces_context_reset_policy`, and focused idle-queue context-reset tests.
