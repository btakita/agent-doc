...
- `preflight.queue_continuation_required == true` — the authoritative "there is agent-drainable work" signal (`#cleardrainsignal`/`#qchurn`). It is `false` when every remaining head is `[clean-session]` (under a live editor-IPC listener) / `[operator-verify]` / inert noise, even while `queue_active == true` and a `queue_trigger` is present. **When it is `false`, do NOT loop and do NOT re-dispatch** — that is a no-op churn cycle, not continuation. This signal is computed in the binary from the same drainability filter the supervisor idle-watch uses, so it holds even if the route-owned supervisor predates that filter; never re-derive drainability by hand from the queue prose. `preflight.queue_drainable_head_count` is the underlying count (`0` ⇒ stop).
...
Skip the auto-loop on any failed closeout, `session-check` interruption, or `lint-gate` block — those need explicit operator attention. Skip when `preflight.queue_active == false` (queue drained or halted) **or `preflight.queue_continuation_required == false`** (no agent-drainable head — only deferred/noise heads remain; looping would just churn a no-op `#qchurn` cycle, so report the deferred/noise heads in one line and stop without pausing or punting). High session-accretion is **not** in that skip list — it is handled by the supervisor `/clear`-and-continue path above, not by stopping.
