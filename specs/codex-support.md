...
3. ✅ Refactored `start.rs::run()`:
   ...
   - Clean exit handling is harness-aware: Codex auto-restarts in resume mode after a normal `codex exec` turn instead of dropping into the Claude-style `Enter`/`q` prompt, but stdin EOF/Ctrl-D and failed resume handoffs restart fresh instead of prompting or chaining another blind resume
