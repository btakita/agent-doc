...
Managed Codex panes do not treat correct process args as capability proof. When a document requests network access, required SSH targets, or extra writable roots, `start` runs live DNS, isolated SSH, and writable-root probes and records `codex_capability_proof` in the session log. Route refuses to trust an older ready pane that lacks a current proof after the latest `session_start`; it restarts the managed session fresh once with the original launch contract, then reroutes or fails closed with the missing proof. `agent-doc session status` surfaces that state.
