...
mod core_tests {
    ...
    #[test]
    fn try_compact_editor_converge_durable_open_zero_live_replica_fails_closed_without_receipt() {
        // P4 authority parity: durable reliable-sync Open remains authoritative even
        // when a recycled controller has not rebuilt any live relay members yet. A
        // socket ACK without a visible-write receipt must not demote that durable Open
        // to disk authority or materialize the compacted candidate behind the editor.
        let dir = TempDir::new().unwrap();
        let agent_doc_dir = dir.path().join(".agent-doc");
        fs::create_dir_all(agent_doc_dir.join("logs")).unwrap();
        fs::create_dir_all(agent_doc_dir.join("visible-write")).unwrap();
        ...
        // Editor opened the doc, then the controller recycled, leaving durable Open
        // liveness over a recovered hub with zero live replicas.
        agent_doc_test_support::seed_durable_open_zero_live_replica(&doc, "intellij:recovering");

        // A receipt-without-content listener ACKs the socket but never proves that the
        // editor made the candidate visible.
        let _listener = agent_doc_test_support::start_receipt_without_content_listener(dir.path());
        agent_doc_test_support::wait_for_live_prompt_drift_listener(dir.path());

        let err = try_editor_converge(&doc, &compacted, &source, "compact")
            .expect_err("durable Open must fail closed without a visible-write receipt");
        assert!(
            err.to_string().contains("no_visible_write_receipt"),
            "the refusal must identify the missing editor-visibility proof: {err:#}"
        );
        assert_eq!(
            fs::read_to_string(&doc).unwrap(),
            source,
            "disk must remain the old non-authoritative projection while durable Open is live"
        );
        let log = fs::read_to_string(agent_doc_dir.join("logs/ops.log")).unwrap();
        assert!(
            log.contains("reason=no_visible_write_receipt"),
            "the missing editor-visibility proof must be observable in ops.log:\n{log}"
        );
        assert!(
            !log.contains("transport=disk_stale_lease")
                && !log.contains("action=demote_phantom_editor"),
            "durable Open must never be demoted merely because the relay currently has zero members:\n{log}"
        );
    }
