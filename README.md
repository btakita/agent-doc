...
- **Codex dispatch-only reroutes trust shared Enter delivery** — if a live Codex pane accepts `agent-doc route --dispatch-only <file>` through the shared tmux text+`Enter` path but hook-visible tracking does not record routed submission proof inside the proof window, route completes with `proof=accepted proof_scope=accepted_only` instead of returning a hard IDE failure. Hook proof is still logged as stronger `dispatch_start` evidence when available.
