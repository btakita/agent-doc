...
- **Normal maintenance covers stale starting actors** — `preflight`, `start`, and `sync` now close stale `starting` controller actors older than one hour when no fresh supervisor heartbeat or live supervisor PID still proves that generation is in progress, then re-emit `session-actors.json` from SQLite. The heavier orphan-file GC and old diagnostic pruning remain on the daily `.agent-doc/gc.stamp` cadence.
