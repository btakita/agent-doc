...
A `finalize` / `write --commit` that reached `committed` with `session-check` OK is a **successful closeout even when the socket IPC degraded** only when delivery was proven through the editor/socket or file-IPC path. Console lines like `IPC socket degraded` and `falling back to file-IPC` describe the transport; they are not failures by themselves when the closeout reached `committed`.
...
Crucially, a degraded-but-proven transport is **not** a reason to stall an active queue drain. When `queue_continuation_required == true`, the binary emits `queue_continuation_guidance` (and `session-check` prints the same line) restating that degraded IPC, a stale/wedged route-owned supervisor, high session-accretion, and `semantic_completion_match` warnings are **not** stop reasons after a proven closeout. The stop conditions are a failed closeout, an unproven IPC/delivery retry condition, a `session-check` interruption, or a `lint-gate` block. A degraded or stale supervisor is its own self-heal / recycle concern; it never licenses the loop to stop. The mechanism depends on the staleness kind (`#supselfheal`): a busy/idle supervisor is freed when the short-TTL drain lease expires, but a stale-**binary** supervisor (older than the installed build) does **not** self-heal by lease expiry — it recycles non-disruptively via the wedge-triggered recycle (a wedged editor-IPC write requests a recycle through the policy owner), an explicit `agent-doc admin recycle`, or a bounded kill+relaunch when the in-place `execve` cannot start. Do not invent a stop reason from proven transport degradation, and do not punt a drainable head to "a fresh session" because the supervisor looked stale. When a stale-**binary** supervisor is genuinely blocking the drain (a `stale_install` warning, or persistent `live_prompt_drift_after_preflight` traced to a finalize client newer than the running supervisor), the recovery is active, not passive (`#staleloop-recycle-restart`): run the install if the binary is stale relative to source (`make install` — `cargo install --path .` + `cargo build --release` + `agent-doc lib-install`), run `agent-doc admin recycle <FILE>`, then **restart the loop** so the turn ends, the supervisor recycles onto the new binary at idle, and the next iteration drains against a matched supervisor. Recycling but then stopping instead of restarting the loop is itself the bug; only stop-and-report if the recycle cannot be initiated.
...
Compact Exchange does not use the normal response IPC `fullContent` path. With a live editor listener it closes through component `op:replace` editor IPC; the guarded direct disk write is only the no-listener fallback plus the standard compact commit/VCS refresh boundary. Stale JetBrains live-buffer/file-cache proof or unproven active-listener apply must fail before document or snapshot replacement, then the operator resolves the IDE buffer or listener and reruns compact. See [compact-exchange.md](compact-exchange.md) and the Compact Exchange section of [jb-cache-conflict.md](jb-cache-conflict.md).
