...
- **Codex forwarded `Ctrl+C` and EOF/Ctrl-D now share the quit menu on the normal keepalive path** — after a stdin-forwarded `Ctrl+C` that actually terminates the child, or a forwarded stdin EOF/Ctrl-D, agent-doc returns to an explicit canonical `Enter`/`q` prompt mode so the operator can intentionally restart fresh or exit the supervisor cleanly even when the parent harness keeps stdin in raw-ish mode
- **Successful routed Codex turns keep their pane attached** — if a routed/editor-owned Codex child already committed a document cycle, a forwarded `Ctrl-D`/stdin EOF no longer drops the supervisor into the quit path and tear downs the claimed tmux pane; agent-doc restarts fresh instead, and that keepalive successor now ignores only the stale inherited pre-prompt `Ctrl-D` byte that belonged to the completed run
- **Pre-prompt Codex `Ctrl-D` exits now recover automatically** — if a fresh/fresh-restart Codex child clean-exits after forwarded `Ctrl-D`/stdin EOF before it ever surfaces an idle prompt, agent-doc now treats that as failed startup provenance, restarts fresh without stopping for the quit prompt, and suppresses only the stale inherited pre-prompt `Ctrl-D` byte on the successor run until a real prompt appears
- **Dispatch-only reroutes now share the same busy-pane guard as managed reroutes** — `agent-doc route --dispatch-only <file>` still ends with a one-shot bare reopen, but it now reuses the same ready/repair/restart checks before injecting into an existing Codex pane. Still-booting or visibly busy panes fail closed instead of taking the older optimistic send path
...
- **Passive editor sync now takes the fast handoff path first** — `sync --no-autostart` reuses the latest matching pane immediately, falls back to an alive exclusive registered pane rooted to the same document when no direct match exists, and only then cold-starts a new pane instead of paying the full supervisor/process-tree recovery cost on every tab switch
- **Stale registry-rebind closeouts no longer strand passive reopen** — `sync --no-autostart` now treats `session_end origin=registry_rebind ...` as a blocker only while the recorded successor pane is still alive and rooted to the same document, so stale handoff provenance cannot leave files like `monsterrodholders.md` permanently passive-blocked
- **Live registry-rebind successors now stay authoritative without PID proof** — sync now accepts an alive `session_end origin=registry_rebind ... next_pane=...` successor as live-owner evidence before falling back to generic same-file process-tree matches, so a rebind handoff does not get downgraded just because supervisor/process ids changed
- **Fresh session-log ownership beats stale process-tree matches** — sync now prefers path/supervisor proof and the latest open session-log owner before generic same-file process-tree fallback, so an older pane cannot steal a just-started reroute back from the fresh pane on `monsterrodholders`-style handoff races
