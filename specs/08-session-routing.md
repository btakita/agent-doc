...
**Supervisor replacement:** Dispatch waits on the target document's supervisor recycle projection. File-scoped lifecycle RPCs must not collapse into the project-wide compatibility flag: multiple supervisors share one project. Own `Started` blocks until own `Settled`; sibling events do not change readiness. Event ingress updates the existing keyed reactive projection and wakes waiting routes without rereading the durable ledger.
...
**Supervisor-proven dispatch rebind (`#supervisorprovenrebind`):** A dispatch-only editor reopen may restore a missing actor/registry projection without an operator force-claim only when one live pane is the unique candidate and two independent document-scoped facts agree: the document's supervisor socket reports a PID inside that pane, and the pane process tree names the same canonical document. Session-log, registry-rebind, or process-tree evidence without the supervisor proof remains diagnostic-only. Any redundant candidate, conflicting live registration, missing proof, or dead winner keeps the existing fail-closed/manual-repair behavior. The rebind is persisted through the normal actor-backed `dispatch_bind` transaction before any prompt is submitted.
...
- The bounded bottom-of-pane idle-chrome window is measured in **meaningful rows, not raw terminal rows** (`#opencodeharnessswitch`). tmux captures the full pane height, and a full-screen TUI may pad the gap between its composer box and its bottom status line with blank rows — OpenCode's cold-start splash leaves roughly sixteen. Taking raw rows first spends the whole window on that padding and leaves a single chrome line, below the harness `min_chrome` floor, so a genuinely idle pane never becomes dispatch-ready and the auto-trigger records `auto_trigger_timeout harness=<harness> reason=no_prompt_after_30s`. Blank rows are filtered before the window is bounded, and one shared scan serves both readiness and stale-busy-cue suppression so the two cannot drift. OpenCode's `opencode` wordmark, drawn from `█`/`▀`/`▄` block glyphs, is idle box art like the composer rules; a line qualifies only when every non-whitespace character is an art glyph.
...
Automatic `resume_latest` focus recovery is still bounded routing, not an implicit operator restart. Its typed `ProvisionOnly` effect must record the observed missing bound pane before consuming the route-owned recent-session-loss projection. The first isolated loss may recover, but once the persisted crash window reaches its threshold the controller fails closed without pane creation or registry rebind. An explicit `restart-supervisor` recovery uses the operator-owned `WaitForReady` policy, records no automatic loss, and remains available.
