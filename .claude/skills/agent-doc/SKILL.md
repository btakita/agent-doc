...
Skip the auto-loop on any failed closeout, `session-check` interruption, or `lint-gate` block — those need explicit operator attention. Skip when `preflight.queue_active == false` (queue drained or halted). High session-accretion is **not** in that skip list — it is handled by the supervisor `/clear`-and-continue path above, not by stopping.
