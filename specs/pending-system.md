...
**Same-cycle backlog-add queue sync (`#pendingaddqueuesync`):** `--backlog-add*` mutations (legacy `--pending-add*` aliases) are applied during finalize/write, after preflight's ordinary backlog→queue sync has already run. In an active go-mode queue whose `agent:backlog` carries a recognized `queue` attribute, closeout appends ids recorded in `cycle_state.pending_added_ids` into `agent:queue` after current-head consumption and before commit. This append-only closeout path preserves the existing runnable queue order, skips done/already-queued/operator-verify ids, updates the snapshot queue region, and does not run for plain persisted-active queues without go so the `#backlog-queue-sync-pending-add-amplification` guard remains intact.
