...
- **Behavior:** Every one of those events reports exactly one observation through `agent_doc_editor_surface_observe_json(project_root, surface_json)`. `surface_json` is an `EditorSurface`: `{ "focused": "<absolute path>", "visible": ["<absolute path>", ...], "columns": [{ "files": [...] }, ...], "force_reconcile": bool }`. Paths are absolute so a derived focus can address the document. A real tab-selection event sets `force_reconcile` so a missing actor/supervisor can still be cold-started when nothing about the surface changed; focus-only activation does not. The receipt is `{ "intent", "idle", "outcome", "error" }`.
- **What plugins must NOT do:** choose between `focus_document_pane` and `sync_tmux_layout`; hold a previous-observation / previous-signature / last-focused-file field; dedup repeat events; run a preserved-layout or lock-contention retry ladder; apply sync-timeout backoff; or report whether tmux has drifted. All of that is derived in `agent-doc-editor-surface` and driven by an `Effect` in `agent-doc-editor-surface-io`, so the rule exists once rather than once per editor. An observation identical to the previous one is idle and costs nothing, which is why repeat events need no plugin-side guard.
...
- **Drift:** whether tmux still matches the reported layout is the controller's observation, never the editor's. The graph fills that side of the mirror itself (`tmux_layout_sync_state` for multi-column surfaces); a plugin must not query it and report the answer back.
- **Debounce:** 100ms, plus a generation guard, on an event loop or scheduler keyed by editor events — never hot polling, spinlocks, or thread-per-delay retry loops. This is the only coalescing a plugin owns: it exists so an event storm reports its final state, not so identical states are suppressed. Report off the UI thread; the derived Project Controller command runs inside the observe call.
- **Project close:** call `agent_doc_editor_surface_forget(project_root)` for every root the plugin observed, so a root's reconciled-layout history does not outlive the editor that produced it.
- **Feedback:** a derived `sync` intent shows the concise inline layout hint; `focus` and `idle` intents show nothing. The full receipt stays in the plugin log for diagnosis.
...
**CRDT-only attached write convergence:** Unless the operator explicitly requested the `--force-disk` recovery escape hatch, a document with an attached CRDT replica must wait for typing quiescence and use the canonical CRDT document as the sole visible mutation plane. Every accepted candidate is first retained in the Lazily deferred-write lineage. When an older delivery is still awaiting acknowledgement, the candidate may be queued once behind it; the final content-qualified ACK cumulatively retires the coalesced prefix, and an identical retry is an idempotent canonical no-op. An accepted CRDT replacement must never be followed by component/full-content IPC for the same candidate; the plugin would report that replay as a new local edit. Disk materialization is only a projection of exact canonical text after every attached replica acknowledges the delivery frontier, and it is refused if canonical text advances before the projection. While an ACK is pending, the binary signals client replay every 250 ms, requests one targeted replica re-register after two seconds, and returns after eight seconds with CRDT + Lazily state retained if the client still cannot converge. It does not stall for a minute, recycle the controller, ask the operator to close the document, or offer a competing disk write. Explicit force-disk writes still pass through the per-document actor, retain their pre-write base and target in Lazily state, and intentionally skip the CRDT gate.
