...
pub(crate) enum BatchProgressDecision {
    Continue,
    StopSourceChanged,
...
pub(crate) fn classify_batch_progress(
...
pub(crate) fn queue_freeze_event(task_count: usize, from_exchange: bool) -> FlowEvent {
    FlowEvent::new(
...
pub(crate) fn source_changed_event(completed_steps: usize, total_steps: usize) -> FlowEvent {
    FlowEvent::new(
        FlowName::OrchestrationBatch,
        FlowStage::QueueFreeze,
        FlowOutcome::Blocked,
...
pub(crate) fn child_closeout_event(child: &BatchChildResult) -> FlowEvent {
    FlowEvent::new(
        FlowName::OrchestrationBatch,
...
pub(crate) fn log_queue_freeze_event(file: &Path, task_count: usize, from_exchange: bool) {
    super::proof::log_flow_event(file, queue_freeze_event(task_count, from_exchange));
}

pub(crate) fn log_source_changed_event(file: &Path, completed_steps: usize, total_steps: usize) {
    super::proof::log_flow_event(file, source_changed_event(completed_steps, total_steps));
}

pub(crate) fn log_child_closeout_event(file: &Path, child: &BatchChildResult) {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct ChildPatchbackNormalization {
    pub(crate) response: String,
    pub(crate) decision: ChildPatchbackNormalizationDecision,
}

pub(crate) fn normalize_child_template_response(response: String) -> ChildPatchbackNormalization {
...
pub(crate) fn child_patchback_normalization_event(
...
pub(crate) fn log_child_patchback_normalization_event(
    file: &Path,
    normalization: &ChildPatchbackNormalization,
) {
