...
- `Clear Session Context` runs `agent-doc session clear <relative-path>` so Codex/Claude clear semantics stay aligned with the binary-owned clear command path while leaving the next `Run Agent Doc` dispatch-only reroute on the same live session. The binary records live pane status for diagnostics, but file-scoped clear does not refuse solely because stale or conservative live evidence says `alive-busy`; it logs that classification and sends the clear command through the resolved live pane or supervisor path.
...
- `Refresh and retry` first runs `agent-doc session status <relative-path>`; if the direct live pane evidence is already `alive-idle` with `prompt_ready=true`, the plugin immediately retries `agent-doc session clear <relative-path>`, otherwise it shows the exact status output and still leaves `Clear Session Context` available as the operator recovery action.
