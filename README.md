...
- **Preflight gate** — `agent-doc preflight` auto-recovers open `response_captured` / `write_applied` cycles, treats `staged snapshot already matches HEAD` as an already-committed no-op closeout instead of `commit_failed`, requires a real pending/capture replay before auto-closing an open `preflight_started` cycle, fails closed on unrecoverable `preflight_started` drift instead of snapshot-committing over newer live content, and emits `effective_tier`, `required_tier`, `suggested_tier`, `model_switch(_tier)`, and `agent_model` for the skill
...
- **Bare-path compatibility alias** — `agent-doc <file>` still routes to `agent-doc run <file>` so older wrappers keep working while the explicit subcommand form remains canonical
