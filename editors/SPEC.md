...
- **Acknowledgement:** For automatic layout sync, the plugin may trust the controller's terminal applied command receipt once the desired projection generation is published; it need not wait for the later tmux effect receipt. Foreground routes instead wait for physical pane count/order and focused-pane convergence before dispatch. Repeating an identical foreground request is a fresh intent generation and physical observation edge, so focus or geometry drift cannot be acknowledged from stale `Converged` or terminal `OperatorOwned` state. This admission rule does not apply to document CRDT delivery, whose closeout barrier still requires the exact editor-visible projection receipt.
...
Automatic tab-to-pane sync is **reported, not planned** (`#jbsurfaceswap` / `#jbpluginlazilyeffects`). A plugin publishes an ordered fact; the controller's process-scoped reactive graph derives the intent and its local `Effect` owns the tmux consequence. Passive spanning-layout observation uses only an already-running Project Controller. The explicit selected-document focus handoff asks the shared native policy to ensure that document's controller is serving, then retries the same ordered fact once.

- **Trigger:** Editor tab selection changes, visible-layout changes, and editor-specific activation (JetBrains focus/mouse activation, VS Code active-editor changes while clicking between already-open split editors).
- **Behavior:** Selection and layout events report the spanning editor surface through `agent_doc_editor_surface_observe_json(project_root, surface_json)`. JetBrains component-focus also republishes that spanning surface because focus-only state cannot promote a stashed pane; its separate focused-document projection preserves mixed-root selection ownership. Native transport adds `(client_id, generation, sequence)`. Passive layout facts use the existing-controller socket without launching. A selected-document focus fact is a latest-wins handoff: if its destination socket is absent or accepts without serving before the focus deadline, the shared controller policy verifies/replaces the same-project controller and the plugin retries that exact fact once. `surface_json` is an `EditorSurface`: `{ "focused": "<absolute path>", "visible": ["<absolute path>", ...], "columns": [{ "files": [...] }, ...], "force_reconcile": bool, "preserve_focus": bool }`. `preserve_focus` marks a structural observation that may reconcile columns but cannot move the active tmux pane; explicit selection/component-focus and missing-owner recovery remain focus-authoritative. Paths are absolute so a derived focus can address the document. The request reply is admission/diagnostic framing; accepted results are also controller state-plane projections.
- **Desktop focus guard:** Automatic focus-only and layout-following effects may select a tmux pane only while the desktop editor is active (or when desktop focus cannot be observed). A delayed editor event received while the operator is working in a terminal or another application is retained as an observation but must not pull tmux focus into the right-hand agent pane. Explicit operator focus and manual layout commands remain authoritative.
...
**CRDT replica transport:** First-party editor plugins send CRDT replica IPC only to the Project Controller socket (`.agent-doc/controller.sock`) using the controller `crdt_replica` envelope. Every editor payload carries the real editor process PID as `editor_pid`. A detached agent-doc `replica_register` is delegated to the relay so that live process-scoped proof may establish editor authority; generic/dead registrations, non-agent-doc markdown, and non-registration detached operations remain refused. Plugins must not connect to per-session supervisor sockets such as `.agent-doc/supervisor/*.sock` for replica register/update/pull/ack/deregister/current-text work. Remote CRDT delivery is event-driven through the shared targeted `EditorIntent` channel and Lazily-backed controller subscriptions; it never writes a per-document event file. Startup/open-document attach and local-delta forwarding may also queue one-shot drains, but plugins must not run a fixed interval pull loop, hot polling loop, spinlock, or thread-per-delay CRDT retry loop. Transport acceptance is provisional until retained-projection reconciliation commits: replacement registration keeps the predecessor membership authoritative, `replica_promote` atomically retires it against the candidate's exact accepted canonical frontier, forced refreshes and ambiguous holds preserve the existing retry projection, and only a committed endpoint clears its backoff state.
