...
- **Queued editor reruns can be superseded** — if a dispatch-only editor rerun is queued behind a busy actor in `agent:queue auto`, repeating `Run Agent Doc` after editing the prompt replaces the sole live route-owned queue prompt. Multi-prompt queues still append so explicit user batches are preserved.
