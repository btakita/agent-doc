...
Managed Codex panes do not treat correct process args as capability proof. When a document requests network access, required SSH targets, explicit extra writable roots, or auto-injected submodule/superproject git metadata roots, `start` runs live network, isolated SSH, and writable-root probes and records `codex_capability_proof` in the session log. Explicit network access requires both host DNS and a bounded `codex exec --json` child probe that runs DNS plus HTTPS from inside the Codex sandbox with the same launch args. Writable-root proof checks launcher write access and a bounded Codex child probe under the same launch args; git metadata roots must prove `index.lock` creation/removal so parent `.git/modules/...` sandbox denial is caught before commit work. Managed non-dispatch route refuses to trust an older ready pane that lacks a current proof after the latest `session_start`; it restarts the managed session fresh once with the original launch contract, then reroutes or fails closed with the missing proof. Editor dispatch-only reroutes still submit the bare reopen into the live pane instead of restarting before submit. `agent-doc session status` surfaces that state.
