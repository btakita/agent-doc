...
- **Template exchange guard** — template writes/compaction repair the safe case where a trailing `## User` / `## Assistant` / `### Re:` block escaped below `<!-- /agent:exchange -->`; ambiguous mixed structure fails closed instead of being committed malformed
