...
/**
 * Consumes the Project Controller lazily state projection into a plugin-facing status label plus
 * the double-append forwarding guard. The Project Controller owns the phase; the plugin observes
 * the lazily mirror and never reads cycle sidecars on the editor hot path.
 */
object TurnStateBridge {
    private val LOG = Logger.getInstance(TurnStateBridge::class.java)
    private val START_SESSION_PANE_ALIAS_REGEX =
        Regex(
            """start_session cross-document actor pane alias: pane (\S+) is already claimed by .*?\bgeneration=(\d+)\s+state=(\S+)""",
            RegexOption.DOT_MATCHES_ALL,
        )
    private val PROJECT_CONTROLLER_COMMAND_FAILED_REGEX =
        Regex(
    ...
    data class TurnStatePresentation(
        ...
        val label: String,
        /**
         * True when forwarding an operator prompt now would collide with an in-flight response (the
         * `live_prompt_drift` double-append guard): the plugin queues it for the next turn instead
    ...
    fun presentationFromDocumentAuthority(
        ...
        if (authorityJson.isNullOrBlank() || authorityJson == "null") return null
        return try {
            val authority = JsonParser.parseString(authorityJson).asJsonObject
            when (authority.get("readiness")?.asString ?: "pending") {
                "pending" -> null
                "unavailable" ->
                    TurnStatePresentation(
                        ...
                        tooltip =
                            "Agent Doc Project Controller is not connected for this document.\n" +
                                (authority.get("error")?.asString
                                    ?: "Start or reconnect the Project Controller."),
                        showBanner = false,
                    )
                else -> {
                    val projected =
                        authority.getAsJsonObject("turn")?.let { presentation(it.toString()) }
                            ?: TurnStatePresentation("", false, showBanner = false)
                    if (projected.label.isNotEmpty()) {
                        projected
                    } else {
                        routeFailurePresentationForFile(filePath) ?: projected
                    }
                }
            }
        } catch (e: Throwable) {
    ...
    fun presentation(json: String): TurnStatePresentation =
        try {
    ...
    private fun steeringLabel(state: String): String? =
        when (state) {
    ...
    private fun routeFailureReason(routeOutput: String): String? {
        val lines = routeOutput.lineSequence().map { it.trim() }.filter { it.isNotEmpty() }.toList()
        val line =
            lines.firstOrNull { it.startsWith("Error:", ignoreCase = true) }
                ...
                ?: lines.firstOrNull()
        return line?.removePrefix("Error:")?.trim()?.ifEmpty { null }
    }
    ...
    private fun compactWhitespace(text: String): String = text.replace(Regex("""\s+"""), " ").trim()
