...
- **Editor document switches focus only after the sync lock** — passive editor sync resolves the focused markdown file through the controller actor binding after acquiring the bounded sync lock, then selects its live pane before prune cleanup or tmux-router reconciliation; if the lock is contended, sync emits the retry marker without partially moving tmux focus to a hidden pane
...
- **Passive sync proceeds around protected closeouts** — when `sync --no-autostart` needs to attach a newly requested pane while some already-visible unwanted pane is protected by an open `preflight_started` / `response_captured` / `write_applied` cycle, agent-doc protects the closeout pane from DETACH but still attaches/focuses the requested document immediately, accepting temporary visible pane growth instead of deferring a different document's sync
...
- **Open closeout panes are protected during sync reconcile** — if an unwanted pane still owns a document with an open `preflight_started`, `response_captured`, or `write_applied` cycle, sync warns and keeps that pane visible instead of stashing it mid-closeout
