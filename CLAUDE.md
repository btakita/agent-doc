...
- **Route readiness stays in the binary** — `route.rs` owns pane prompt detection and trigger acceptance. Keep that logic resilient to shell startup noise / echoed command text, and do not push harness-readiness heuristics into the skill layer.
...
- **Successful routed Codex turns keep their pane** — when a routed/editor-owned Codex child already committed a document cycle, `start.rs` must restart fresh instead of entering the Ctrl-D quit prompt path so a successful turn cannot immediately close the claimed tmux pane. Keep that rule aligned in `start.rs`, `README.md`, `SPEC.md`, and the supervisor/Codex specs.
