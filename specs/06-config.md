...
For both Claude and Codex, `agent_args` is the harness-neutral override and takes precedence over harness-specific aliases. When the active document lives in a git submodule whose git metadata resolves outside the working tree, agent-doc supplements the resolved args with harness-native `--add-dir` entries for those external git directories.
