...
- JetBrains handles socket `save_document` messages as typed editor-owned saves. The handler waits for typing idle, locates the open IntelliJ `Document`, calls `FileDocumentManager.saveDocument(document)`, and writes ack-content for the supplied `patch_id`. It must not replace document text, use VFS binary writes, or revive reconnect reread repair on this path.
