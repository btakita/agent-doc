...
- `[layout-sync]` (`EditorTabSyncListener`) — `selectionChanged`, immediate command-plane `focus`, the single surface-graph observation/reconcile request (including the columns derived from `[layout-detect]`), its typed receipt, plus debounce/guard/deferred/timeout/generation diagnostics. `LayoutChangeDetector` only reports structural changes into this graph.
