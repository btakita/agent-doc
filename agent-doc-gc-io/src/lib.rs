...
pub struct GcControllerConfig {
    pub stale_starting_after: Duration,
    pub dead_actor_prune_after: Duration,
    pub stale_preparing_controller_after: Duration,
}
...
impl Default for GcControllerConfig {
    fn default() -> Self {
        Self {
            ...
            dead_actor_prune_after: Duration::from_secs(7 * 86400),
            stale_preparing_controller_after: Duration::from_secs(10),
        }
    }
}

pub trait GcControllerEffects {
...
impl GcControllerEffects for NoopGcControllerEffects {
    fn close_stale_starting_actors(
        &mut self,
        _project_root: &Path,
...
pub fn run_with_controller_effects(
    root: Option<&Path>,
    dry_run: bool,
    controller: &mut impl GcControllerEffects,
    controller_config: GcControllerConfig,
) -> Result<GcResult> {
    ...
    let (actors_closed, actors_kept) = controller.close_stale_starting_actors(
        &project_root,
        controller_config.stale_starting_after,
        dry_run,
    )?;
    if actors_closed > 0 {
        eprintln!(
    ...
    let (dead_pane_closed, dead_pane_kept) = controller.close_stale_dead_pane_actors(
        &project_root,
        dry_run,
        "gc",
    ...
    let (actors_pruned, actors_prune_kept) = controller.prune_dead_actors(
        &project_root,
        controller_config.dead_actor_prune_after,
        dry_run,
    )?;
    if actors_pruned > 0 {
        eprintln!(
    ...
    let (preparing_reaped, preparing_kept) = controller.terminate_stale_preparing_controllers(
        &project_root,
    ...
    let (orphan_reaped, orphan_kept) = controller.reap_orphaned_preparing_controllers(
        &project_root,
        controller_config.stale_preparing_controller_after,
        dry_run,
    )?;
    ...
    let (xproj_reaped, xproj_kept) = controller.reap_orphaned_preparing_controllers_all_projects(
        controller_config.stale_preparing_controller_after,
        dry_run,
        "gc",
    )?;
