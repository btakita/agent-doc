...
- **Invalid YAML frontmatter now surfaces a document-targeted startup error instead of raw parser noise or silent sync skips.** `frontmatter.rs` now wraps parse failures with the document path plus a repair hint for the `--- ... ---` block. `start.rs` and `route.rs` use that wrapper directly, so malformed frontmatter fails closed with actionable feedback. `sync.rs` now logs the same contextual warning during file resolution and auto-start instead of silently dropping the file from the current pass. Added regression coverage for the shared parse wrapper, `start`, and sync-phase error context.
