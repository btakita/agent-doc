...
pub fn try_migrate_renamed(doc: &Path) -> Result<bool> {
    ...
    for entry in std::fs::read_dir(&snap_dir)? {
        ...
        if let Ok(snap_content) = std::fs::read_to_string(&path)
            && let Ok((snap_fm, _)) = agent_doc_frontmatter::frontmatter::parse(&snap_content)
...
mod tests {
    ...
    fn document_crdt_save_persists_legacy_and_overlay_state() {
        ...
        assert_eq!(
            agent_doc_merge::crdt::CrdtDoc::decode_state(&loaded_legacy)
                .unwrap()
                .to_text(),
            markdown
        );
    ...
    fn crdt_merge_base_state_prefers_matching_overlay_projection() {
        ...
        let stale_legacy =
            agent_doc_merge::crdt::CrdtDoc::from_text("stale legacy text").encode_state();
    ...
    fn crdt_merge_base_state_rebuilds_stale_overlay_after_first_fallback() {
        ...
        assert_eq!(
            agent_doc_merge::crdt::CrdtDoc::decode_state(&rebuilt.state)
                .unwrap()
                .to_text(),
            baseline
        );
...
";
        ...
        let merged = agent_doc_merge::crdt::merge(Some(&base.state), &ours, &theirs).unwrap();
...
";
    ...
    fn overlay_ahead_single_char_preserved_through_merge() {
        ...
        let merged = agent_doc_merge::crdt::merge(Some(&base.state), &baseline, &live).unwrap();
        assert!(
    ...
    fn overlay_concurrent_divergent_both_survive_no_dup() {
        ...
        let merged = agent_doc_merge::crdt::merge(Some(&base.state), &ours, &live).unwrap();
        assert!(
