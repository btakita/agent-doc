...
//! is authoritative … snapshots are backup, not hot-path authority" contract in
//! `AGENTS.md`.
...
///
/// Restricting to `head ∩ live_file` keeps legitimate edits from tripping the
/// guard:
...
mod tests {
    ...
    fn flags_synthetic_reap_frontmatter_drop() {
        // aboobakkar shape: HEAD + live file both carry full frontmatter, the
        // corrupt snapshot (to_commit) dropped agent/claude_model/claude_args/
        // pending_done_guard.
    ...
    fn allows_pending_prompt_without_frontmatter_change() {
        // Normal pending-prompt: live file has an unanswered prompt in the body,
        // snapshot is the smaller last-answered baseline, frontmatter identical.
        let snapshot = FULL_FM;
        let live_with_prompt = "---\nagent_doc_session: s\nagent: claude\nclaude_model: opus\nagent_doc_format: template\nagent_doc_write: crdt\nclaude_args: --x\npending_done_guard: warn\n---\n\n## Exchange\n\n<!-- agent:exchange patch=append -->\nAnswer this please.\n<!-- /agent:exchange -->\n";
        assert!(dropped_committed_frontmatter_keys(snapshot, FULL_FM, live_with_prompt).is_empty());
    }

    #[test]
    fn ignores_operator_added_key() {
        // Operator adds `no_mcp` in the working tree; snapshot/HEAD predate it.
        let live_added = "---\nagent_doc_session: s\nagent: claude\nagent_doc_format: template\nagent_doc_write: crdt\nno_mcp: true\n---\n";
        let snapshot = "---\nagent_doc_session: s\nagent: claude\nagent_doc_format: template\nagent_doc_write: crdt\n---\n";
        let head = snapshot;
        assert!(dropped_committed_frontmatter_keys(snapshot, head, live_added).is_empty());
    }

    #[test]
    fn ignores_operator_removed_key() {
        // Operator deletes `claude_args` from the working tree; the new snapshot
        // legitimately lacks it. HEAD still had it, but live_file no longer does,
        // so it is not a corrupt drop.
        let live_removed = "---\nagent_doc_session: s\nagent: claude\nagent_doc_format: template\nagent_doc_write: crdt\n---\n";
        let snapshot = live_removed;
        let head = "---\nagent_doc_session: s\nagent: claude\nagent_doc_format: template\nagent_doc_write: crdt\nclaude_args: --x\n---\n";
        assert!(dropped_committed_frontmatter_keys(snapshot, head, live_removed).is_empty());
    }

    #[test]
    fn overlay_restores_dropped_frontmatter_preserving_body() {
        // Corrupt snapshot: stripped frontmatter + empty exchange baseline.
        // Live file: full frontmatter. Overlay must yield full frontmatter with
        // the snapshot's (baseline) body — i.e. the correct scaffold shape.
    ...
    fn untracked_first_commit_has_no_head_regression() {
        // Empty HEAD (untracked / first commit): nothing was committed to regress.
