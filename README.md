...
- **Dispatch-only reroutes fail closed on still-booting panes** — `agent-doc route --dispatch-only <file>` still sends the bare reopen immediately for normal live panes, but if the latest open session-log run is still only a fresh `*_start`/`*_restart` with no ready prompt yet, route does one short ready probe and refuses to inject into that still-booting pane
