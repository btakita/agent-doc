...
**Backlog capture rule:** if the response creates concrete follow-up work, add it to `agent:backlog` in the same cycle. Plain `--backlog-add` prepends and is for work more urgent or relevant than the existing head. During an active drain, an adjacent, equal-priority, or lower-priority follow-up must not preempt the existing head; pass `--backlog-queue-placement append`. When one write carries several plain adds, they land in flag order top-down and the queue mirror matches that order. For a specific interleave use `--backlog-add-after <id>` or `--backlog-add-before <id>`; these anchors determine both backlog and queue-mirror order, and queue placement does not override them (`#queuemirrororder`). To fix an already-queued order, use `--backlog-reorder`: it cascades into the queue mirror, permuting only the named live heads among their existing slots (`agent-doc queue sync` cannot — it skips ids already present). If the item is only a recommendation, include `[recommended]`.
...
**Agent harnesses own full-suite verification:** if you changed code, tests, build logic, or instruction surfaces, run the full project verification suite explicitly after edits and before `finalize` / `write --commit`. Preserve the command's exit status explicitly across wrappers and pipes and, when the runner exposes one, independently assert its runner-native failure count. Never replace either proof with a generic output grep copied from another runner. Do not rely on a pre-commit hook. Do not waive red suites as "unrelated" or "flaky". See [runbooks/persist-closeout.md](runbooks/persist-closeout.md).
