...
- **Passive sync also avoids visible pane growth around protected closeouts** — when `sync --no-autostart` would have to attach a newly requested pane while some already-visible unwanted pane is still protected by an open `preflight_started` / `response_captured` / `write_applied` cycle, agent-doc now preserves the current visible layout and warns instead of expanding the `agent-doc` window into a 3+ pane mix that it already knows cannot detach cleanly; if the requested focus file is already visible, sync still hands off pane focus inside that preserved layout
