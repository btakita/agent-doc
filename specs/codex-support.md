...
Managed Codex panes do not treat correct process args as capability proof. When a document requests network access, required SSH targets, explicit extra writable roots, or auto-injected submodule/superproject git metadata roots, `start` records `codex_capability_proof status=pending`, launches the Codex pane, and runs live network, isolated SSH, and writable-root probes asynchronously. Explicit network access requires host DNS and either a bounded `codex exec --json` child probe that runs DNS plus HTTPS from inside the Codex sandbox with the same launch args or a same-process cached success for the same harness, command, probe args, and environment. Cached network proof events report `network_probe=child_dns_https_cached`. Writable-root proof checks launcher write access and a bounded Codex child probe under the same launch args; git metadata roots must prove `index.lock` creation/removal so parent `.git/modules/...` sandbox denial is caught before commit work. Successful writable-root proof records a normalized `writable_root_contract` fingerprint. Prompt dispatch through supervisor IPC, auto-trigger, managed non-dispatch route, and dispatch-only route is gated until the latest-start proof records `status=proven` for the current root contract; a failed proof marks the actor blocked and disables prompt dispatch, but does **not** terminate the live hosted child (`#tsiftmdcrash`): the dispatch gate alone prevents unsafe auto-dispatch, so a false-positive background probe (for example a flaky `opencode run` network proof child timing out) can no longer SIGTERM a healthy interactive session the operator is actively using. The supervisor logs `<harness>_capability_proof_live_child_preserved reason=dispatch_gated_not_killed`; the operator fixes the environment / stops / restarts to re-prove. A ready pane with no current proof after the latest `session_start`, or with proof for an older writable-root contract, is still unproven and may be restarted fresh by managed non-dispatch route. `agent-doc session status` surfaces that state.

A transient capability-proof failure (for example a brief network blip that times out the bounded child probe) no longer permanently wedges the session. The proof thread retries the probe with exponential back-off before committing the dispatch gate to `Failed`; while retrying it keeps the gate at `Pending` (gated but recoverable) and records `codex_capability_proof status=retry attempt=<n>/<max>`. The retry budget, base back-off, and child probe timeout are configurable in frontmatter (`managed_proof_max_attempts`, `managed_proof_retry_backoff_secs`, `managed_proof_probe_timeout_secs`) and in `.agent-doc/config.toml` (same keys), with frontmatter taking precedence; defaults are 3 attempts, 2s base back-off, and a 45s probe timeout. Only after the budget is exhausted does the gate become `Failed` and mark the actor blocked. The live hosted child is left running (`#tsiftmdcrash`) — the `Failed` gate is itself a complete dispatch block, so there is no need to SIGTERM a healthy interactive session the operator is using.
...
3. ✅ Refactored `start.rs::run()`:
   ...
   - A definitive Codex `No saved session found with ID <id>` response clears `resume` only when the document still names that exact ID, then immediately launches and triggers a fresh session instead of feeding the stale ID into crash backoff. Before retrying a cached resume launch, the supervisor also rechecks document authority; if the operator already removed `resume`, it discards the cached resume arguments and relaunches fresh. Crash backoff remains Ctrl+C-interruptible while no child owns stdin.
