...
- **Binary-side verification:** The binary verifies the ack-content sidecar by checking that each non-blank `normalize_prefix_lines` target appears with a `❯ ` prefix (using `trimEnd()` comparison). If any target is missing its prefix, the binary falls back to `content_ours` as the snapshot source instead of the sidecar, then re-applies the same `normalize_prefix_lines` to that fallback before saving the snapshot. The binary may send that fallback back to the editor as a full-content IPC repair only when the repair payload carries `expected_content_hash` / `expected_content_len` for the exact rejected sidecar buffer and the current disk buffer still matches that bad state before delivery. If the proof is stale, the binary must skip full-content redelivery and use the normal snapshot/disk repair path or a narrow normalization/reposition patch. This prevents normalization failures in the plugin or in an intermediate fallback snapshot from propagating into the committed snapshot without risking a stale full-document editor replacement.
- **Typed repair decision:** Before repairing a sidecar-normalization divergence or duplicate-response IPC snapshot, the binary resolves a typed decision with the repaired snapshot content, snapshot source, disk-repair reason, editor bad-state length/hash fingerprint, and an explicit `redeliver_editor` boolean. Plugins must treat `fullContent` redelivery as valid only when the payload still passes the bad-state `expected_content_hash` / `expected_content_len` proof; skipped or rejected redelivery leaves the binary to repair disk/snapshot state without editor replacement.
...
- `expected_content_hash` / `expected_content_len` (optional for legacy payloads, required for new `fullContent` payloads): the `flow::document_mutation` source-buffer proof for the text the binary read before building `fullContent`. Before emitting a `fullContent` IPC payload, the binary also compares this caller-supplied source buffer with the current disk buffer and logs `full_content_source_buffer_rejected` plus a `flow_event` if the replacement is already stale. Plugins must compare the current editor buffer's UTF-8 SHA-256 and byte length to these fields before applying a full-document replacement; mismatches reject the patch without acknowledgement.
