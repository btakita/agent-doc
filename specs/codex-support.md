...
3. ✅ Refactored `start.rs::run()`:
   ...
   - Clean exit handling is harness-aware: Codex auto-restarts in resume mode after a normal `codex exec` turn instead of dropping into the Claude-style `Enter`/`q` prompt, and forwarded stdin EOF/Ctrl-D normally surfaces that same restart-fresh-or-quit menu again so the operator can intentionally leave the supervisor. A stdin-forwarded `Ctrl+C` that actually terminates the child uses that same quit menu instead of being treated like a crash, while route/plugin-injected interrupts that bypass stdin forwarding stay on the automatic recovery path. Those supervisor prompts must force a canonical local tty mode for the prompt read itself instead of trusting the inherited parent harness stdin flags, so Enter continues to work even when the outer binding session left stdin raw-ish. If that child run already committed an `agent-doc` document cycle, or if a fresh/fresh-restart child exits before ever surfacing a prompt, the supervisor still restarts fresh after Ctrl-D and suppresses only stale inherited pre-prompt `Ctrl-D` bytes until the successor shows a real prompt. Failed resume handoffs still stop chaining blind resumes by restarting fresh on the first failure and escalating to the resume-failure prompt on repeated failures, but prompt-time EOF on that path now also restarts fresh rather than quitting
