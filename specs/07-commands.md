...
Malformed YAML frontmatter is not silently dropped during resolution or auto-start. Sync logs a warning that includes the file path plus a repair hint for the `--- ... ---` block, mirrors that warning into the document's `agent:status` component when present so editor-driven auto-starts have a visible in-document error, then skips that file for the current pass. Once the file parses again, sync clears only the status text it previously wrote for that warning.
