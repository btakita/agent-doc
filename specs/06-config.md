...
- `agent_doc_queue_context_reset = <bool>` — explicit opt-in for accretion-driven fresh-context handoff in direct `run`, Codex Stop-hook continuation, and Codex supervisor idle-queue receiver paths (`#nm1x-no-preempt-clear`, `#nm1x-codex-clear-parity`). Default off. When enabled and a reset reason is active, the supervisor idle-queue watch may send the harness-native clear command for an ordinary Codex queue head, but only after the pane is idle and route/turn/pending-clear gates pass. Explicit operator clears and explicit `[clean-session]` queue heads remain separate clear sources. A document-frontmatter `agent_doc_queue_context_reset: true` takes precedence over this project setting.
- `agent_doc_clear_threshold = <0..100>` — context-usage percentage at or above which an opted-in direct `run` / Codex Stop-hook continuation / Codex idle-queue receiver requests fresh context before the next dispatch (`#clear-opt-in-threshold`). Resolution: per-document frontmatter `agent_doc_clear_threshold`, then this project setting, then the built-in default of `50`; values are clamped to `0..=100`. The binary owns the threshold value so every harness/editor shares it and surfaces it on the preflight `session_accretion.clear_threshold` field (`session_accretion::clear_threshold_for_doc`). Supported live transcript sources include Claude `usage` JSONL and Codex `token_count` JSONL; missing or unsupported transcript data fails safe (`pct=none clear=false`). This is the configurable companion to `agent_doc_queue_context_reset` (the on/off opt-in).
