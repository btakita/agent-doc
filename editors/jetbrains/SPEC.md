...
- `Run Agent Doc` and `Clear Session Context` are serialized per document in the JetBrains action layer before either command reaches the binary. A normal clear is rejected while a Run route process is dispatching; `Interrupt and Clear Session Context` remains the explicit discard path. If Run is clicked while a normal clear command is already running, the latest Run intent is queued and starts only after the clear completes synchronously. A clear accepted for deferred delivery by the binary does not release the queued Run immediately.
