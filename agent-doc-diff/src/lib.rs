...
use agent_doc_prompt_lines::{
    ...
    line_looks_like_soft_prompt_request, line_looks_like_targeted_prompt_prefix_repair_start,
    looks_like_imperative_directive, normalize_imperative_candidate,
    normalized_prompt_preview_line, parse_markdown_list_item, parse_slash_command_candidate,
    text_line_looks_like_prompt_target, trimmed_line_looks_like_markdown_list_item,
};
...
fn block_looks_like_prompt_target(block: &str) -> bool {
    block.lines().any(text_line_looks_like_prompt_target)
}
...
pub fn detect_exchange_compaction_request(diff: &str) -> bool {
    ...
    for line in diff.lines() {
        ...
        if !line.starts_with('+')
            ...
            || in_fence
            || content.starts_with('>')
            || trimmed_line_looks_like_markdown_list_item(trimmed)
        {
