...
## Degraded IPC is still a successful closeout (`#degraded-ipc-no-stall`)

A `finalize` / `write --commit` that reached `committed` with `session-check` OK is a **successful closeout even when the socket IPC degraded** and it fell back to the file-IPC patch queue. Console lines like `IPC socket degraded`, `falling back to file-IPC`, `IPC timeout — response saved as patch`, `IPC proof insufficient … recovery=direct_write_fallback`, and `CRDT merge successful` describe the **transport**, not a failure: the file-IPC patch queue + CRDT merge already carried the response to `HEAD`. Verify the commit landed (`session-check` `committed`, one clean `### Re:` heading, clean worktree) and treat the cycle as complete.

Crucially, a degraded transport is **not** a reason to stall an active queue drain. When `queue_continuation_required == true`, the binary emits `queue_continuation_guidance` (and `session-check` prints the same line) restating that degraded IPC, a stale/wedged route-owned supervisor, IPC ack timeouts on the last write, high session-accretion, and `semantic_completion_match` warnings are **not** stop reasons — the only stop conditions are a failed closeout, a `session-check` interruption, or a `lint-gate` block. A degraded or stale supervisor is its own self-heal / recycle concern (the short-TTL drain lease frees it to resume); it never licenses the loop to stop. Do not invent a stop reason from the transport, and do not punt a drainable head to "a fresh session" because the supervisor looked stale.
