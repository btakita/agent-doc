...
| `heuristics.rs`, response text normalization, closeout response/done-signal parsing | `agent-doc-turn` | Extracted. Pending-capture recommendation detection, explicit no-follow-up response detection, future-work response signals, append response heading normalization, imperative response contract classification, closeout response text normalization, closeout prompt/response text matching, response/done-signal parsing, reaped queue-directive response-loss detection, free-text queue response-proof/residue detection, partial-closeout shipped/remaining-work detection, blocked follow-up closeout detection, gated-phase split body detection, and queue-audit partial-completion collapse detection are turn closeout policy; orchestration only applies the resulting signal to guards and file/cycle adapters. |
