...
**Fenced transcript cleanup (`#backlogfencedlogcleanup`):** Lazy backfill removes a complete top-level fenced terminal transcript whose content contains the `› agent-doc ...` invocation prompt. This is captured session output rather than tracked work. Ordinary fenced prose, indented item continuations, and unmatched fences are preserved; cleanup fails closed unless the matching fence and transcript signature are both present.
