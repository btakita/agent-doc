...
**Authoritative actor dispatch:** When the durable actor store has a healthy authoritative record for the document generation, route must treat that actor pane as the dispatch target and send the bare reopen through supervisor IPC. Supervisor-side PTY injection must normalize submit newlines to carriage return before writing to the child, so `Run Agent Doc` and other routed reopen paths behave like a real Enter key instead of drafting a literal newline into the managed harness composer. In that state, `sessions.json` is only a projection/binding helper: route may refresh the registry to match the actor pane, but it must not re-elect ownership from live-owner heuristics, same-file associated panes, or opportunistic registry rebinds.
