...
- **Managed Codex panes must prove requested capabilities before managed reuse** — when a Codex document requests network access, required SSH targets, or extra writable roots through `--add-dir`, `start` records a `codex_capability_proof` session-log event after live network/SSH/write probes succeed. For `codex_network_access: enabled`, that includes host DNS plus a bounded `codex exec --json` child probe that must run DNS and HTTPS from inside the Codex sandbox. The proof event includes `timings_ms` for host DNS, child network, SSH, launcher writable-root, child writable-root, and total proof time so slow starts identify the expensive phase. Managed non-dispatch route will not blindly reuse an older ready Codex pane without that current proof; it restarts the managed session fresh once with the original launch contract, then reroutes or fails with the missing capability. Dispatch-only editor reroutes keep their one-shot live-pane submit behavior. `agent-doc session status` reports the proof as `proven`, `missing`, or `not_required`.
