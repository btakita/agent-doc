...
- `finalize` / `write --commit` must reject stale snapshot/CRDT reset drift before applying granular `--pending-*`, `--review-*`, or `--status` mutations. A closeout that cannot safely place the exchange response must not still mutate backlog/review/status state, because that creates active queue work without the response/proof that explains it.
