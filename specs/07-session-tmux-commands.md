...
- A controller `editor_route` invocation with non-empty selected text is a separate active-turn steering admission. It must require the authoritative supervisor actor to be `busy`, preserve the selection bytes exactly, use the supplied steering id for retry deduplication, and return the supervisor's typed `delivered`/`duplicate` acknowledgement without entering the route/startup/new-trigger state machine. Invocations without selected text retain the existing busy-pane refusal and must never inject a new reopen trigger into the active turn.
