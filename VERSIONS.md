...
- **Model-projected snapshot baseline — Rungs 2-4 opt-in cutover (`#mps`).** Re-homes the merge baseline (the `--baseline-file` common ancestor the finalize merge uses) from the on-disk `.md` file onto the structured document model, behind `AGENT_DOC_MPS=1` so it can be exercised end-to-end and reverted instantly (flag off == byte-for-byte current behavior). Rung 2 (pin): `preflight`'s `save_baseline_content` also persists the baseline as an overlay sidecar (`baselines/<hash>.overlay.yrs`) via new `snapshot::save_baseline_model`, logging `mps_baseline_pin`. Rung 3 (flip): `write::read_explicit_baseline` sources the base by projecting that overlay (`snapshot::load_baseline_model`) when the flag is on, cross-checking against — and failing safe to — the legacy `.md`, logging `mps_baseline_resolve source=model|md_fallback` and a loud `mps_baseline_divergence` (with first-differing byte) on any mismatch. Rung 4 (derive): the `.md` baseline becomes the derived cross-check cache, no longer the read authority (the file is retained until live divergence logs are clean; full removal is a later step). The baseline overlay is a separate sidecar from the crdt-runtime overlay so the cutover never perturbs the stream/crdt write merge, and is migrated on rename. Coverage: `snapshot::tests::mps_baseline_model_*` (round-trip, absent→None, projects-when-md-diverges, idempotent delete, first-diff-byte). Verified end-to-end through the installed binary: preflight pins the overlay + emits `mps_baseline_pin`, finalize resolves `source=model diverged=false`, response merges and commits. FlowCore hot-path token budget bumped +2 (`reason=no_model|model_error`) with audit.
