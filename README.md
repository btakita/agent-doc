...
- **Codex EOF/Ctrl-D now restores the quit menu on the normal keepalive path** — after a forwarded stdin EOF/Ctrl-D, agent-doc returns to the cooked-mode `Enter`/`q` prompt so the operator can intentionally restart fresh or exit the supervisor cleanly
