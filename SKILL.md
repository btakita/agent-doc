...
- **Harness readiness is binary-owned** — when `agent-doc route` / `start` dispatches into an existing pane, do not treat a generic shell echo or `>` prompt as proof that the harness is ready. Rely on the binary's prompt detection and closeout guards instead of ad hoc prompt heuristics in the skill layer.
...
- **MCP auth / OAuth steps are sub-steps, not closeout boundaries** — if a tool asks the user to authenticate, opens a browser, or otherwise pauses for MCP auth, treat that as part of the same managed `agent-doc` turn. Resume the turn after the auth step and still finish through `agent-doc finalize <FILE>` or `agent-doc write --commit <FILE>` plus `agent-doc session-check <FILE>`; do not claim success just because the tool step completed.
...
- **Path-scoped repo commits must fail closed on stage drift** — when a turn narrows ordinary repo `commit + push` work to specific non-session paths, resolve the exact intended path set first, run stage commands only for that set, stop immediately if any `git add` / stage step fails, then verify the staged diff still matches the intended set before `git commit`. Use an explicit pathspec commit or equivalent isolated-index strategy so unrelated pre-staged entries cannot leak into the commit.
...
- **Stale startup-miss supersession follows the current file owner** — when the document has already been rebound to a newer pane/session, treat the older pane's `startup_miss` marker as stale once the current owner logs a newer `session_start`; do not keep the clear-path tied to the superseded `session_id`.
...
**Session document staging rule:** if the turn also asks for ordinary repo `commit + push`, keep the session document out of that manual git commit. The allowed order is: resolve the intended non-session path set, stage only that set, stop if any stage step fails, verify `git diff --cached --name-only` (or a stricter submodule-pointer inspection) still matches the intended set, commit only that validated non-session set, run `agent-doc finalize <FILE>` or `agent-doc write --commit <FILE>` so the session document crosses its own binary-owned commit boundary, then push after closeout.
...
**Codex hook backstop:** the Codex install also writes `.codex/hooks.json` plus `.codex/config.toml` with `features.codex_hooks = true`. Those `UserPromptSubmit` / `Stop` hooks track the active `agent-doc` file across nested `.agent-doc` roots in the same workspace. On `Stop`, the hook first tries to finish the response cycle deterministically from `last_assistant_message`, but only when that payload validates as a single assistant closeout. If the payload looks like a transcript dump or full exchange patchback, the hook stores it only as diagnostics and blocks/fails closed instead of replaying it. If `last_assistant_message` is empty because Codex stopped after a tool-only/authentication step (for example an MCP OAuth / `authenticate` flow), the hook must still block, save diagnostics, and force the turn back onto the normal `finalize` / `write --commit` path instead of letting the auth step masquerade as a completed session response. Treat that as a safety backstop, not a replacement for explicitly running `finalize` / `write --commit`.
