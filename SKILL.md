...
- **Preflight is binary-owned (`#preflightinbinary`)** — the turn context must contain the `[agent-doc] cycle contract (preflight already ran in the binary; ...)` success marker. Consume the contract sealed by that marker; never shell back to `agent-doc preflight <FILE>` or poll authority from the model turn. If the marker is absent, stop as a harness-admission failure so the hook/controller can be repaired.
