...
- Markdown `fileOpened` is also a visible-surface source edge. IDEA may restore split containers before their files and then emit neither a selection nor another structural event, so the listener re-observes the completed surface after the file lifecycle edge; it does not invoke tmux directly.
- Every markdown selection and visible-layout change reports one editor-surface observation containing the focused document, visible documents, open documents, and detected columns. Native transport adds an ordered reload generation/cursor and publishes it to an existing controller. The controller's process-scoped reactive graph derives idle or layout reconciliation and owns the tmux effect. One editor surface has exactly one active controller-root subscription: after successfully publishing through a different root, the listener generation-fenced-forgets every prior root so two retained controller projections cannot reconcile the same tmux window from incompatible layouts. A failed forget stays retryable on the next successful publication. Selection and component-focus submit one generation-fenced `focus_document_pane` intent to the focused document's own controller root; component-focus is selection-only because it cannot change the visible surface. This separate handoff is required when one visible editor surface spans a superproject document and a submodule document. The plugin never chooses a pane or tmux window.
...
- The plugin applies only a short event-storm debounce and generation guard before reporting the final surface. Its separate focus lane keeps only an in-memory generation and a micro-coalescing delay; the Project Controller provides the project-scoped latest-wins command semantics. It stores no previous layout signature, last-focused file, pending retry, or durable controller copy.
...
- Manual `Sync Tmux Layout` submits `agent-doc.sync_tmux_layout.v1` to the Project Controller command plane
  ...
  any failure is shown with the controller diagnostic. Automatic editor-surface
