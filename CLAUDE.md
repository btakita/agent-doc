...
- **Selective commit is conservative** — `git::commit()` stages the snapshot, not arbitrary working-tree drift. The only absorbable out-of-band repair path is narrow agent-owned drift (`status`, `### Re:` response-block insertion, pending-ID superset) when the redacted component structure still matches. Plain user prompts must remain uncommitted. Already-committed historical response-block drift may repair the snapshot only when the working tree matches `HEAD` modulo transient boundary / `(HEAD)` markers. Even under extreme snapshot/file drift, tracked docs must not wholesale re-sync the snapshot from the live file — reserve that bootstrap escape hatch for untracked scaffold snapshots only. After a successful commit, boundary cleanup must collapse the snapshot / user-facing document state back to the same clean shape as the committed blob so post-success `(HEAD)` / boundary-only churn does not linger in the working tree or editor.
