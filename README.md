...
- **Existing managed reroutes are supervisor-owned** — once a Claude/Codex session is running under `agent-doc start`, route/reopen traffic goes through supervisor IPC instead of direct pane `send-keys`, and the supervisor now re-injects those commands back through the claimed pane's tmux input path rather than writing raw reopen bytes into the child PTY. That keeps editor reroutes, `/clear`, queue-dispatch, and restart handoffs on one real Enter-delivery boundary
...
- **Dispatch-only reroutes keep post-clear editor runs on the live session** — after `agent-doc session clear <file>`, the next editor-triggered `agent-doc route --dispatch-only <file>` still sends the bare `agent-doc <FILE>` reopen through the existing supervisor-owned Enter submit path; only the managed non-dispatch route keeps the tracked `/clear` fresh-restart policy
