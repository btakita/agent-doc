...
- **Codex forwarded `Ctrl+C` and EOF/Ctrl-D now share the quit menu on the normal keepalive path** — after a stdin-forwarded `Ctrl+C` that actually terminates the child, or a forwarded stdin EOF/Ctrl-D, agent-doc returns to the cooked-mode `Enter`/`q` prompt so the operator can intentionally restart fresh or exit the supervisor cleanly
