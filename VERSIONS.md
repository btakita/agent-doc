...
- **Finalize-tolerance + post-commit-repair groundwork; behavior-flip rungs held for operator confirmation (`#fintol`/`#pcwc`/`#rtwbcast`).** Three phases from `plan-clean-exchange-run.md` landed only their pure, seam-isolated, non-behavior-changing groundwork after the wiring was found to conflict with a deliberate shipped invariant:
  ...
  - `#pcwc` — `emit_postcommit_worktree_check` stays a detection + warn signal. Auto-repair (reconcile the worktree to HEAD on `match=false`) was prototyped then reverted: a post-commit worktree legitimately differs from HEAD whenever a concurrent edit was carried forward uncommitted, so a blanket reconcile clobbers that edit. Safe auto-repair needs a lost-committed-content discriminator plus operator confirmation.
