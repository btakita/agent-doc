...
- **Queued editor reruns can be superseded** — if a dispatch-only editor rerun is queued behind a busy actor in `agent:queue auto`, repeating `Run Agent Doc` after editing the prompt replaces the sole live route-owned queue prompt. Multi-prompt queues still append so explicit user batches are preserved. If an explicit `Run Agent Doc` finds a busy authoritative actor and the document already has a startable inactive `agent:queue auto` head, route promotes that head to `queue_active: true`, syncs the snapshot, and lets the supervisor idle-queue watch drain it once the pane is idle instead of failing as a silent no-op.
