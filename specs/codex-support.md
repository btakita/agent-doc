...
Managed Codex panes do not treat correct process args as capability proof. When a document requests network access, required SSH targets, explicit extra writable roots, or auto-injected submodule/superproject git metadata roots, `start` records `codex_capability_proof status=pending`, launches the Codex pane, and runs live network, isolated SSH, and writable-root probes asynchronously. Explicit network access requires both host DNS and a bounded `codex exec --json` child probe that runs DNS plus HTTPS from inside the Codex sandbox with the same launch args. Writable-root proof checks launcher write access and a bounded Codex child probe under the same launch args; git metadata roots must prove `index.lock` creation/removal so parent `.git/modules/...` sandbox denial is caught before commit work. Successful writable-root proof records a normalized `writable_root_contract` fingerprint. Prompt dispatch through supervisor IPC, auto-trigger, managed non-dispatch route, and dispatch-only route is gated until the latest-start proof records `status=proven` for the current root contract; failed proof marks the actor blocked and terminates the unusable child. A ready pane with no current proof after the latest `session_start`, or with proof for an older writable-root contract, is still unproven and may be restarted fresh by managed non-dispatch route. `agent-doc session status` surfaces that state.
