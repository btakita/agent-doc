...
- **Fallback tmux reroutes now submit in one tmux invocation instead of split text/Enter writes.** The latest JetBrains `Run Agent Doc` repro still drafted `agent-doc …` into the managed Codex composer even after the supervisor IPC newline normalization fix, which meant the remaining failure surface was the non-authoritative tmux fallback path. That path reused `tmux-router`'s split literal-text plus later `Enter` submit helper, leaving a race where managed panes could observe the reopen text without the submit. `tmux-router::Tmux::send_keys()` now batches the literal reopen and `Enter` submit into one tmux invocation, and `tmux-router` added a regression that locks that batched command shape. Updated the routing spec so non-supervisor reroutes keep the same single-invocation submit contract as supervisor IPC.
