...
**Typed editor-owned save:** The `save_document` IPC operation is a flush request, not a document replacement. It may only save the already-open editor buffer through the editor's native save API (`FileDocumentManager.saveDocument` in JetBrains, `TextDocument.save` in VS Code), after the active-typing debounce succeeds. The plugin must then write the saved buffer to `.agent-doc/ack-content/<patch_id>.md` so the binary can prove what reached disk. It must not call `Document.setText`, `WorkspaceEdit`, VFS binary-content replacement, reconnect reread repair, or synthesize `fullContent` delivery from this path. VS Code receives the same typed operation via `.agent-doc/patches/save-document.signal` because it does not run the socket listener.
