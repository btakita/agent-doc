...
| Controller RPC/CAS | `agent-doc-controller` | Active. Controller stores/apply decisions; it does not own supervisor policy. Pure dispatch admission helpers for in-flight coalescing, operator reopen classification, stale-generation redirect parsing, recycle debounce, and force-recycle bypass now live in `agent-doc-controller`; orchestration imports them directly and the RPC layer remains only the socket/process adapter. |
...
| `route`, `project_controller`, `state_store` | `agent-doc-controller` | Route should become an adapter to controller admission and actor bindings. Started: dispatch coalescing and stale-generation retry parsing moved to `agent-doc-controller::dispatch`; controller recycle debounce and force bypass moved to `agent-doc-controller::recycle`; stale-queue recovery still depends on orchestration flow outcome and should be split next. |
