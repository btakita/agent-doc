...
- **Codex stdin `Ctrl+C`/`Ctrl+Z` are handled as supervisor control keys instead of composer text (`#codexctrlz`).** The stdin writer now strips Codex `Ctrl+C` and `Ctrl+Z` before any PTY write, logs `tmux_input_event ... op=codex_operator_control key=Ctrl-C|Ctrl-Z`, maps `Ctrl+C` to child SIGINT, maps `Ctrl+Z` to a managed SIGTERM stop plus the restart-fresh-or-quit prompt, and preserves `Ctrl+D` as the explicit EOF path. This prevents operator control-key bytes from surfacing as literal `^C`/`^Z` glyphs in the Codex composer or transcript. Coverage: `codex_operator_control_strips_ctrl_c_and_ctrl_z_from_forwarded_bytes`, `non_codex_operator_control_preserves_control_bytes`, `restart_continue_strategy_prompts_after_ctrl_z_operator_stop`.
- **Session-check backlog guards now fail open on component-parse noise.** The shadow backlog detector and external done-archive replay proof no longer hard-block an otherwise valid closeout when their auxiliary scan input trips on unmatched archived/transcript component markers; they emit warnings and let the rest of session-check's structural guards decide. This prevents a committed response from being reported as failed solely because backlog guard side scans encountered stale exchange marker text outside the live component model.
- **Queue consume now ignores discarded stale captures and undrainable leading heads before positional fallback.** Empty-response recovery writes that only retire a stale capture no longer reuse that discarded response body as proof to strike the next free-text queue head. Queue consumption now accepts response bodies only from live capture states (`captured`, `write_applied`, or `replayed`) and refuses the positional free-text fallback when the active capture is terminal but still carries stale response text. The free-text fallback also now checks the same drainable-head filter used by preflight/session-check, so an unrelated response cannot strike an undrainable SimWorld note that sits before the real drainable `do [#id]` head. This closes the repair path where `write --commit` retired a stale response for one queue item and then consumed the following SimWorld prompt without answering it. Coverage: `queue_consume_ignores_discarded_capture_instead_of_striking_head_by_position`, `consume_decision_keeps_undrainable_free_text_before_drainable_id_head`.
- **`agent:exchange queue` now moves newly typed exchange prompts into the running queue.** Preflight now treats `<!-- agent:exchange queue -->` as a prompt intake surface: newly-added exchange prompt blocks are appended/prepended/synced into `agent:queue` using the same `queue` attribute modes, removed from the exchange tail so they do not remain as unanswered direct prompts, and mirrored into the snapshot. The preflight diff suppresses those moved prompt targets so the current queue head keeps running instead of being interrupted by the newly queued follow-up. Coverage: `run_queue_maintenance_moves_exchange_queue_prompts_into_active_queue`, `queued_exchange_prompt_diff_suppression_matches_moved_prompt_targets`.
- **Queue consume now targets the response-heading prompt instead of blindly striking the first live free-text item.** When a closeout response explicitly names a later live queue prompt, the planner now consumes that prompt by queue ordinal and mirrors the same prompt into the snapshot instead of applying the free-text fallback to position zero. Id-backed targets still require an explicit completion signal, so halt/log responses cannot strike a directive by heading alone. This closes the post-restart mis-strike where a PCP/Codex response consumed the skipped `#qrefmisstrike` prose item. Coverage: `queue_consume_targets_response_heading_instead_of_first_live_prompt`.
- **Queue consume now adopts settled free-text head edits at closeout instead of failing on stale snapshot text.** When a captured response explicitly targets the live queue head, the planner now treats a single non-id-backed snapshot head that is a prefix of the live prompt as the same adopted free-text item, strikes the live head, and updates the snapshot to the live queue body. This closes the post-restart closeout failure where preflight adopted an edited head but queue consume still compared against the stale snapshot prefix. Coverage: `queue_consume_adopts_settled_free_text_head_edit_from_live_document`.
- **Queue live-buffer convergence now uses node patches to avoid unrelated queue churn.** Preflight still sends the legacy full queue-body convergence patch for older plugins, but current JetBrains plugins receive queue `node_patches` as well and skip the full component replacement for `agent:queue`. Queue maintenance can now strike/insert/move only the AST items it owns while preserving unrelated operator edits in the live queue buffer. Coverage: `queue_convergence_node_patches_preserve_unrelated_live_queue_edits`.
- **Codex/JB Run Agent Doc now submits with a real Enter key instead of a CR-byte newline.** The direct-pane Codex route path now sends literal reopen text and then a named tmux `Enter` key in one ordered tmux command sequence, trimming any trailing line endings before delivery. OpenCode stays on Kitty Return and Claude stays on the existing carriage-return byte path. Operator diagnostics now report `submit_mode=tmux_literal_text_enter_key` / `routed_trigger_enter_key` for Codex so future logs cannot confuse this with the old `tmux_literal_cr` path. Coverage: `codex_submit_mode_uses_literal_text_plus_real_enter_key`, `routed_trigger_submit_diagnostic_names_codex_real_enter_key`, `direct_pane_submit_mode_is_harness_specific`, updated idle-queue submit-mode regression.
- **Codex/JB session restarts now clear stale-supervisor queue pauses instead of leaving the queue stalled.** A `queue_paused` controller record whose reason is a stale-supervisor churn-stop (`stale supervisor`, `supervisor_binary_stale`, or `churn-stop ... needs operator recycle`) is now auto-resumed when a new authoritative `start_session` generation is accepted for that document. Deliberate operator pauses stay fail-closed. The controller logs `stale_supervisor_pause_repaired ... action=session_start` and the restarted generation can dispatch immediately instead of requiring a manual queue resume/recycle after the Codex session comes back. Coverage: `session_start_clears_stale_supervisor_queue_pause_before_dispatch`, plus the existing stale-supervisor dispatch marker regression.
...
- **Codex/Claude idle queue drains now submit already-drafted restart triggers instead of stalling.** When the supervisor idle-watch sees its drain payload or `/clear` already visible in the owned pane composer, it no longer treats that as handled for Codex/Claude. It sends one bare `Enter` keyed by the live queue head, logs `idle_queue_watch_resubmit ... action=enter_key`, and keeps OpenCode on the existing dedup-only path. This closes the restart path where a Codex session came back with `agent-doc <FILE>` drafted but the queue stopped until the operator pressed Enter. Coverage: `pending_payload_enter_resubmit_is_scoped_and_one_shot`, existing `idle_queue` filtered tests.
...
- **JB `Run Agent Doc` on Codex now gets a one-shot Enter re-submit (`#jbcodexsubmit`).** Codex is not owned-PTY managed, so the JB action routes through the direct-pane submit path, where the Codex TUI composer can leave the routed trigger drafted when the text and its trailing carriage return arrive as one `send-keys` payload (the operator had to press Enter manually to start the queue). `route.rs::send_command_unchecked` now, when a Codex direct-pane submit times out with the trigger still visible, sends a separate bare `Enter` key event and re-polls the acceptance window exactly once, logging `route_submit_resubmit ... action=enter_key result=accepted|still_visible`. Scoped strictly to Codex (claude/opencode unchanged) and bounded to one retry so it never loops on a stuck pane. Coverage: `direct_pane_codex_resubmit_only_on_timeout_with_trigger_visible`; the polling loop is factored into `poll_direct_pane_acceptance`. Live end-to-end verification is operator-drive (check `ops.log` for `route_submit_resubmit action=enter_key result=accepted`).
- **JB Stale File Cache recovery now converges through editor IPC first (`#q7jm`).** The `live_prompt_drift_after_preflight` auto-recovery path no longer writes the adopted snapshot directly to disk while a JetBrains listener is active. It now sends component `op: "replace"` convergence patches through the editor, verifies ack-content against the adopted snapshot, and logs `transport=editor_ipc`; the direct disk write remains only as `transport=disk_fallback` and keeps the `[jbstalecache] auto_recovery_disk_write_during_ipc_listener` risk marker. JetBrains and VS Code patch appliers now honor explicit component `op` overrides ahead of marker `patch=append` / `mode=append`, so convergence can replace an append-mode exchange body without duplicating the response. Coverage: `try_auto_recover_live_prompt_drift_prefers_editor_ipc_when_listener_active`, `live_prompt_drift_convergence_patches_builds_replace_patch_for_exchange`, and editor-side `op` override wiring tests.
