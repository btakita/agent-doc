...
- **Accretion guard** — `agent-doc plan` now blocks another normal implementation turn when the same document has clearly accreted too much exchange or restart churn; the emitted blocker tells the caller to compact the exchange and/or restart from the current committed boundary first
