...
- `route --dispatch-only` still uses a one-shot reopen instead of the managed acceptance/cycle-ack path, but for any existing live managed session that one-shot reopen must submit directly through the resolved pane's tmux input path instead of routing back through supervisor IPC or writing raw bytes into the child PTY. Editor callers add `--plain-trigger`, which makes the one-shot reopen the plain `agent-doc <FILE>` prompt even when the document's harness normally uses slash commands. That live-pane submit must use the same literal-text submit followed by a short delayed `Enter` tmux boundary as file-scoped `session clear`. It must reuse the same bounded repair/restart checks before injecting into an existing pane, but it must not impose an extra direct-pane "dispatch-ready prompt" gate that file-scoped clear does not have. A tracked Codex `/clear` or missing managed capability proof may still force a fresh restart on the managed non-dispatch route, but dispatch-only editor reroutes must keep sending the plain reopen into the live session after `session clear` or a missing `codex_capability_proof`.
- When Codex hook tracking is visible for a dispatch-only reroute, a live-pane or supervisor-backed bare reopen is not allowed to return silent success on acceptance alone. If the command leaves the input surface but Codex never records a routed submission proof within the bounded wait, route must fail once with a stage-specific "accepted but unproven" error instead of looking non-responsive. This proof gate applies after any dispatch-only submit helper, including ready authoritative actor reroutes that do not enter the startup prompt gate.
...
- When the controller record and healthy supervisor runtime still report an authoritative actor as `starting`, dispatch-only route must keep using the same direct-pane submit boundary as `session clear` instead of focusing and refusing injection. If the actor pane is visibly dispatch-ready, route may also promote the controller lifecycle to `ready`, but submission must not depend on a prompt probe in a split pane. This must not create, kill, or re-elect panes from the surrounding editor split.
...
- When `route --dispatch-only` has a live authoritative actor pane but the supervisor socket no longer reports a healthy runtime or actor state, route must log the degraded authoritative decision and may still submit directly through that same authoritative pane when the pane matches the current registered/live owner binding. That fallback exists to keep editor reroutes aligned with the already-working file-scoped `session clear` direct-pane path instead of silently dropping to stale registry heuristics. The degraded fallback must skip the managed capability proof gate because the supervisor is unreachable and cannot re-establish the proof; a missing or previously-failed proof must not block dispatch when the pane is still the authoritative binding.
- When the degraded authoritative actor path (or any busy-pane dispatch-only path) finds the pane still busy after auto-fix, route must attempt interrupt recovery for OpenCode as well as Codex. For OpenCode, the interrupt sequence sends Escape twice and then waits for an idle prompt. This handles the post-`Clear Session Context` scenario where the agent-doc child process is still running and OpenCode is showing a progress bar.
