...
- **Idle queue restart drains ordinary JetBrains heads as triggers, not clears.** Added a monsterrodholders regression proving a Codex idle-queue restart drain for an ordinary `Run Agent Doc` head sends `agent-doc <FILE>` with no `/clear`; the supervisor spec now matches the code path where only explicit operator clears and `[clean-session]` heads can interleave a context reset.
...
- **JetBrains Run Agent Doc now trusts the shared Enter submit path and stops threshold-clear churn (`#jbsimpleroute`).** Dispatch-only reroutes no longer fail the IDE action after tmux accepts the shared text+`Enter` submit merely because dispatch-start proof did not arrive inside the proof window; they complete as `proof=accepted proof_scope=accepted_only`, matching the simpler cross-harness delivery contract. The supervisor idle-queue watch also no longer sends transcript-threshold `/clear` commands for ordinary queue heads, even when `agent_doc_queue_context_reset` is enabled; only explicit operator clears and explicit `[clean-session]` heads can make the supervisor clear. Coverage: dispatch-only accepted-delivery policy tests, `clean_session_head_forces_context_reset_policy`, and focused idle-queue context-reset tests.
