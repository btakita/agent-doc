...
- File-scoped `agent-doc session clear <FILE>` must treat a stale `starting` actor projection plus prompt-only document drift as clearable when direct live-pane evidence is available and no protected prompt-input state is visible. Clear is an explicit operator context-reset request; stale startup projection and post-commit user edits are not enough to block the `/clear` submit boundary.
