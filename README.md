...
- **Editor sync lock contention is bounded** — if an older sync process is stuck or orphaned, later selection/layout sync commands wait only for the contention budget, log a retry marker, and return without auto-start, tmux reconciliation, or hidden-pane focus changes so the editor can retry the latest selection
