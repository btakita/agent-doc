...
- Every markdown selection, component-focus, and visible-layout change reports one editor-surface observation containing the focused document, visible documents, open documents, and detected columns. The native reactive graph derives idle, focus, or layout reconciliation from that state; the plugin does not choose or enqueue an imperative tmux command.
...
- The plugin applies only a short event-storm debounce and generation guard before reporting the final surface. It stores no previous layout signature, last-focused file, command-in-flight flag, pending retry, or focus lease.
