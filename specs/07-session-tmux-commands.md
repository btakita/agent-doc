...
- Prompt-bearing one-shot delivery into an existing authoritative actor must still produce a new per-document cycle acknowledgment. Pane acceptance cannot prove that a long-lived harness loaded newly installed admission hooks. A missing acknowledgment fails closed; Claude Code and Codex receive one fresh supervisor restart and retry so their startup hook snapshot is reloaded.
...
- Once hook, actor, or pane-state evidence proves the fresh dispatch started, a slow harness that has not opened its agent-doc cycle by the later cycle-ack window remains an in-flight successful dispatch. Route keeps the authoritative pane and must not record or reap it as a `startup_miss`; the eventual cycle acknowledgment still clears any older miss marker.
