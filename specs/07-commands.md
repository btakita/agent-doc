...
1. Compute diff → 2. Resolve document mode from frontmatter (`resolve_mode()`, default template) → 3. Build the matching append/template prompt (diff + full doc) → 4. Branch if `-b` → 5. Send to agent (and for Codex resumed sessions, if the returned tool transcript shows either the local-browser/CDP EPERM signature on `127.0.0.1:9222` / `localhost:9222` or a target-specific required-SSH failure for frontmatter `required_ssh_targets`, discard that stale resumed attempt and retry once with a fresh `codex exec`; required SSH targets are also probed locally before trusting the session) → 6. Durably capture the final parsed response in `.agent-doc/captures/<doc-hash>/<cycle-id>.json` → 7. Apply the response through the matching append/template write path → 8. Update session resume ID → 9. Persist the final post-write cycle state as `write_applied` → 10. Save snapshot → 11. Run the shared strict post-write closeout helper for git-backed runs: commit, require the cycle to be closed, retry once if the snapshot still differs from `HEAD`, then enforce `session-check`
...
5. If `sessions.json` points at a different **alive** pane but no live owner can still be proven for the document, consult the supervisor before treating it as stale:
   ...
   - **Unreachable** or **No socket** → before any fail-closed/replacement decision, consult the session log's latest open pane provenance. If the log still shows an alive latest-open pane for that session, focus and reuse that pane even when process-tree ownership proof is currently missing. If that same alive pane still owns the active startup-miss marker for the document, fail closed instead of rebinding to a fresh pane. If the alive registered pane is stranded in a stash window, the supervisor is unavailable, and both startup-miss + session-log provenance already prove that the old owner is closed, clear that stale stash registration and continue in the current pane instead of leaving start blocked behind the dead stash binding. Only when neither live-owner proof nor alive latest-open session-log provenance exists may start treat the registration as stale and continue in the current pane
...
10. Exec the configured harness (replaces process). For Codex clean exits, the supervisor still auto-restarts in resume mode by default, but if a fresh/fresh-restart child exits cleanly before it ever surfaces an idle prompt, treat that as failed startup provenance and restart fresh instead of chaining `--continue`. Also, if a forwarded `Ctrl-D`/stdin-EOF prompt fires immediately after a fresh start (within the early-start grace window) and the follow-up prompt itself sees EOF, treat that as a fresh restart instead of an intentional quit so transient pane-move/input races do not close the claimed tmux pane.
...
4. If the registered pane is alive, route first tries **path provenance** from `sessions.json`: the canonical document-path entry must still point at that pane, the pane process tree must still contain the recorded supervisor PID, and the supervisor socket must report the same `supervisor_instance_id`. Only when that top-down proof fails does route fall back to tmux process-tree file-path matching and supervisor-PID recovery. Route only auto-picks a winner when that evidence is decisive: a single provable owner overall, or a single provable owner in the active tmux window while every other candidate is already stashed. Otherwise it fails closed with an explicit ambiguity report that lists every candidate pane plus direct `tmux capture-pane`, `agent-doc claim --pane`, and `tmux kill-pane` follow-up commands. When a winner is found, managed route must first see the harness's idle prompt in that pane; only then does it send the harness trigger via `send_keys`, run the Enter verification loop (polls for command text disappearance every 300ms, retries Enter on each poll, up to 5s timeout), and focus the pane. For routed dispatch, "idle prompt" means an empty composer: Codex/Claude prompt lines that already contain drafted user text are not safe dispatch targets, even if they still begin with the prompt glyph. For Codex, interactive terminal substates such as `reverse-i-search` and queue-only composer states such as `tab to queue message` are explicit non-idle blockers, so route must stop waiting and fall into the normal scoped-fix / bounded-restart recovery instead of burning the full idle timeout. **Dispatch-only** keeps a stricter minimal contract for editor hotkeys: it still sends only the bare `agent-doc <FILE>` reopen, but it does so as a one-shot reopen with no Enter-retry acceptance polling and no busy-pane recovery choreography. If the current pane capture already shows an explicit blocker such as `reverse-i-search`, shell history search, a queued draft, or an active permission prompt, dispatch-only must fail closed before sending anything else. Trigger verification must also treat a visually wrapped `agent-doc <FILE>` line as still pending input; route must not declare success merely because the absolute path wrapped across multiple physical tmux lines. Claude routes send `/agent-doc <FILE>`. Codex routes also send only the bare `agent-doc <FILE>` reopen, even when the document still has unresolved prompt-bearing drift on top of a closed cycle. The document diff remains the source of truth for that follow-up work; route must not paste extra prompt lines into the Codex composer because a multiline payload can be interpreted as ordinary chat instead of re-entering the binary-owned document cycle. Route must validate that final Codex payload immediately before `send-keys` and fail closed if it is multiline or otherwise differs from the bare reopen command, rather than surfacing a later ambiguous startup-miss timeout. When the repo-local Codex `UserPromptSubmit` hook is installed, route also requires a second-stage submission proof for Codex reroutes: after tmux accepts the bare reopen, route should prefer an exact same-prompt hook match as the strongest "consumed" proof, but any newer tracked prompt state for that same document still counts as submission proof before the later cycle-ack health check. That lets route distinguish "drafted in tmux", "accepted but no submission proof", "submitted", and "consumed but no document cycle ever started" without collapsing hook timing races back into a false failure. Prompt-bearing rerun gating is body-only: YAML frontmatter metadata churn such as `agent: codex`, session ids, and model-tier annotations must not be treated as `content_edit` / `prompt_target` work that blocks a routed reopen. The same gate must still catch a plain prose user follow-up appended at the live `agent:exchange` tail immediately before `<!-- /agent:exchange -->`, even when that line has no `❯ ` prefix yet, does not end in `?`, and does not start with an imperative verb; route must not silently downgrade that shape to "already running" just because the prompt classifier missed it. Claude idle detection must treat the newer double-chevron composer chrome (for example `⏵⏵ ... (shift+tab to cycle)`) as prompt-ready state, not as busy output. If the pane never becomes idle and the document still has unresolved prompt-bearing drift, route must run the same scoped repair pass as `agent-doc fix <FILE>` once, then fully re-resolve and retry the route before surfacing a fail-closed busy-session error. When that scoped fix is a no-op and the same-document supervisor is still healthy, route now skips the old "inject into the busy pane, wait, then maybe restart" ladder and instead performs one bounded **fresh** supervisor restart before one final reroute. If that fresh reroute still lands in a same-document Codex pane that remains busy, route may send one bounded interrupt sequence to clear stale shell/composer state, starting with a short `C-g` probe so shell history-search substates can recover before the broader `Escape` + `C-c` fallback. Route then waits again for an actually empty prompt and reruns the same bare reopen once before the final fail-closed error. That final closeout must preserve the last recovery-stage detail: if the bounded interrupt still leaves a specific blocker such as `reverse-i-search`, route must surface that blocker instead of reusing the stale pre-interrupt `timeout` reason. Only restartable/degraded supervisors are eligible for a one-shot in-place restart before retry, and if any restart hands ownership to a different pane route must wait for the replacement pane to become the proved live owner before re-running dispatch. If the pane is proven live for the same document and there is no prompt-bearing drift to dispatch, route may focus that already-running session and return success without injecting a duplicate reopen.
...
**Session validation:** If `tmux_session` references a non-existent tmux session, route logs a warning, ignores the stale pin, and resolves a live target from the most specific visible project scope it can prove: explicit window/session context first, then the shared `.agent-doc` root implied by the reported layout/focus files (or the target file's own nearest `.agent-doc` root when no layout is reported), then the current tmux session, and finally an already-alive harness fallback session. Mixed-root editor layouts therefore stay pinned to the shared workspace root instead of whichever nested repo launched the reroute. If the only remaining target is a dead implicit fallback name (for example `"claude"` / `"codex"`), route fails closed instead of creating that session implicitly.
...
**HEAD marker:** `(HEAD)` is transient with respect to the committed blob and snapshot. `agent-doc commit` strips it before staging, and post-commit cleanup strips it from the snapshot. The working tree may briefly retain `(HEAD)` during editor-coordinated repositioning, but strict closeout is allowed to rewrite the live file back to committed `HEAD` when the only remaining drift is agent-owned transient churn. Preflight still classifies pure `(HEAD)` / boundary differences as `boundary_artifact`, and a boundary-artifact-only turn is normalized back to `no_changes` / already-committed closeout before `preflight_started` is recorded.

**Post-commit cleanup:** After a successful commit, the **snapshot** is normalized to the same clean single-boundary shape as the committed blob (no `(HEAD)`, single boundary). The **working tree** is still repositioned for the normal live-editor case, but successful closeout must also prove the on-disk file is not left dirty by agent-owned transient churn. If the remaining drift from `HEAD` is only closeout artifact state such as `(HEAD)` response-heading attribution or stale/fresh boundary movement, post-commit cleanup must rewrite the live file back to the committed `HEAD` blob, refresh CRDT/editor sidecars, and leave the owning repo worktree clean. Only genuine later user edits may remain uncommitted. The IPC reposition signal includes `preserve_head: true` for the normal best-effort case.
...
`agent-doc sync [--col <FILES>,...] [--col <FILES>,...] [--window W] [--focus FILE] [--no-autostart]` — declarative 2D layout sync.

Mirrors a columnar editor layout in tmux. Each `--col` is a comma-separated list of files. Columns arrange left-to-right; files stack top-to-bottom within each column. When no `--col` args are provided, sync falls back to the recorded `.agent-doc/last_layout.json` for the resolved sync scope and replays that layout.

**Pre-sync file resolution:** Before the layout algorithm runs, sync parses file paths from `--col` args and resolves each file. Files without a session UUID in frontmatter are treated as **unmanaged** and skipped (no auto-initialization of frontmatter). Only `agent-doc claim` adds session UUIDs. Files with session UUIDs are always treated as **registered**, even if the registry entry was pruned (dead pane). This enables the declarative layout flow: navigating to a file in a split creates a tmux pane regardless of registry state. When the caller explicitly passes `--no-autostart`, sync still stays non-destructive around live or ambiguous owners, but it may cold-start a pane after it proves there is no live owner left to rescue. Editor-plugin startup/layout listeners use `--no-autostart` so passive editor churn cannot replace visible panes while ownership proof is uncertain, yet a cleanly exited document can still come back into the `agent-doc` window on selection. When the visible columns span multiple nested project roots, sync must first synthesize a per-run tmux-router registry keyed by session id and populated from each document's own nearest `.agent-doc/sessions.json`; it must not ask tmux-router to resolve every file through the caller's current project root. That synthetic registry keeps cross-repo layouts stable when the editor shows documents from both the workspace root and a child repo. A registered pane is not reusable merely because the pane id is alive; it must still prove live ownership for that document before sync may trust it as the current owner. If that proof weakens but the alive pane still shows live Codex drafted input or another interactive Codex composer/search state, sync must fail closed for that document and record explicit `registered_pane_protected ... action=fail_closed` provenance instead of emitting synthetic `registered_pane_missing` / `session_end origin=sync_missing_pane` or provisioning a replacement pane era on top of the live draft. The same fail-closed rule now applies when the latest session log still shows that exact alive pane as the document's newest open owner, even if process-tree/root heuristics have not caught up yet: sync must treat that as a live-owner ambiguity window, not as permission to write synthetic pane loss. If the synthetic registry finds the same pane id claimed by multiple visible documents, it must fail closed before handing that registry to tmux-router: keep the duplicate pane only when exactly one claimant still proves live ownership, or, if none prove live ownership, only when exactly one claimant uniquely matches the pane's project root. Ambiguous duplicates such as `claudescore.md` and `claudescore-3.md` both pointing at the same child-repo pane must be dropped from the synthetic registry entirely so tmux-router rehydrates a distinct pane instead of aliasing two visible documents onto one live pane. Sync also maintains a per-run pane reservation set: once a live pane is accepted for one visible file in the current reconciliation pass, later files in that same pass must treat that pane as unavailable, even if their registry or associated-pane heuristics still point at it. A duplicate claimant falls back to recovery / auto-start instead of silently aliasing two documents onto one pane. The same fail-closed rule applies after tmux-router returns its layout result: if post-sync registration sees one pane assigned to multiple documents, it must refuse to mirror that duplicate into `sessions.json` unless exactly one claimant matches the pane's own nearest project root or already proves live ownership, and it should prune any losing stale duplicate binding instead of preserving a second cross-root alias. That post-sync registration step must also fail closed for single-document geometry-only assignments whenever the document is already under startup-miss or repeated recent `missing_pane` recovery protection: if sync intentionally skipped auto-start for a fail-closed document, it must not immediately rebind whatever spare pane tmux-router left in that column unless that pane now proves live ownership for the document. More generally, when sync deliberately leaves a managed file unresolved for the current cycle, it must not let tmux-router donate a same-column or spare visible pane to that blocked file at all. Before sync provisions a replacement pane for a managed file, it must first try the shared associated-pane recovery path: collect any panes that still prove ownership for that document session via tmux process-tree match on the file path, supervisor child-PID fallback, or the owning session log's latest still-open pane when that pane is still alive and its current project root matches the document root. That latest-open session-log proof is the fail-safe path for visible panes whose foreground argv no longer advertises the file and whose supervisor socket is temporarily unavailable; sync must prefer reuse or fail-closed ambiguity over synthetic `registered_pane_missing` replacement in that shape. If multiple associated panes remain, sync must fail closed for that file (skip auto-start and log the ambiguity) instead of creating yet another pane on top. Any associated pane already reserved earlier in the same sync run is filtered out before ambiguity resolution so a sibling document cannot keep stealing it back. For managed files whose registered pane is in a stash window, sync **rescues** the pane back to the agent-doc window via guarded `join-pane`, choosing the left or right edge from the requested column so the recovered pane becomes visible without swapping another live pane back into stash. If the registered pane is no longer alive but tmux still retains it as a dead pane, sync must capture tmux's `pane_dead_status`, persist a recent pane tail for provenance, and record the current cycle phase before it treats that pane as lost. If the registered pane cannot be found anywhere and sync is about to replace it, sync first tries to finish any already-open closeout before it writes synthetic pane-loss provenance: `response_captured` must replay from the durable capture through the normal write/commit boundary, and `write_applied` must finish the missing commit boundary when the live file/snapshot already prove the response landed. Only after that best-effort closeout recovery does sync append the synthetic `supervisor_exit code=missing_pane` (or the dead-pane equivalent) plus `session_end origin=sync_missing_pane`. If that closeout recovery itself fails, sync must fail closed for that file: keep the durable capture, record the repair-needed reason, and skip replacement auto-start until the document is repaired instead of provisioning another pane on top of the unresolved closeout. If the cycle is merely a stale `preflight_started`, sync still applies only the narrow stale-preflight repair path. Recovery failures must be logged with the pane-loss provenance and must leave the durable capture available for later `preflight` / `repair`. Before sync auto-starts any replacement pane, it must consult that same session-log history: if two or more `missing_pane` recovery events landed within the last 10 minutes, sync must skip auto-start and leave the document in a stable manual-recovery state instead of cascading more replacement panes. In `--no-autostart` mode, sync must add one more passive-safety gate before cold-starting: it may only provision when there is no unresolved startup-miss and the latest session log shows the previous session has already closed, and it must still refuse that cold-start when the latest closeout was merely `session_end origin=registry_rebind` because that indicates a newer pane era may still own the document elsewhere. Only after those recovery/fail-closed checks pass, or if no alive pane exists at all, does sync auto-start a fresh Claude session (via `route::auto_start()`).
...
**Blank `--window` safety:** Empty or whitespace-only `--window` values are normalized to "unset" before layout repair, auto-start session scoping, stash rescue, and `tmux_router::sync`. A blank scope must never be treated as a real target window or session override, because that can widen reconcile scope beyond the intended IDE window. When `--window` is omitted entirely, sync must first derive the scope from the visible document set's shared `.agent-doc` root (when one exists) and use that root's live `tmux_session` pin ahead of the ambient client session. Only after that root-aware lookup fails may windowless sync fall back to the caller's current tmux session, and it still must not silently inherit the currently focused window if that window is `stash` or another non-layout surface.

**Column memory:** `.agent-doc/last_layout.json` persists a column→agent-doc mapping across syncs. When a column has no agent doc (user switches to a non-session file), sync substitutes the last known agent doc for that column index. Empty `--col` placeholders are preserved long enough for that substitution, so a right-hand markdown file can still restore a remembered left-hand pane instead of collapsing to one tmux pane. The state file is written after each successful sync for columns that contain an agent doc. When no `--col` args are provided later, that same file becomes the default layout input for the next sync. The round-trip contract is: `apply_column_memory` restores empty placeholders from the persisted layout → tmux-router reconciles the expanded columns → `build_layout_state` records the new state → next sync cycle's `apply_column_memory` or no-arg `agent-doc sync` uses that state. Cross-root documents (e.g., `src/boost-client/tasks/monsterrodholders.md` alongside a root-level session) participate in column memory the same way: the canonical path stored in `last_layout.json` resolves through the same `first_agent_doc_in_col` session-UUID check regardless of which project root the file belongs to. The saved layout file is scoped to the same shared `.agent-doc` root that sync uses for session/window resolution; it is not blindly anchored to the caller's CWD.
...
**Template structure guard (tail-patch):** After patch application and again after any merge, template writes fail closed if live conversation content (`## User`, `## Assistant`, `### Re:`, `❯ ` prompts) would land outside `<!-- agent:exchange -->`. The normal write path does not silently swallow ambiguous escaped tails; it rejects the write with a precise error identifying the first escaped line. One narrow repair is now part of the write-time canonicalization: when the parser failure is specifically `closing marker <!-- /agent:exchange --> without matching open`, the document still has the real exchange opener, and the text between the first and second exchange close markers is safe conversation content, the binary must move that escaped content back inside the real exchange block, drop the stray duplicate close marker, and then re-run the same guard. Comment-only notes or other non-conversation scratch content without escaped conversation headings are allowed outside `agent:exchange`. Explicit `agent-doc repair` can still move safe escaped conversation tails back inside exchange for recovery purposes and applies the same duplicate-close repair path.

**Duplicate exchange opener guard:** When a template/CRDT session document gains a second `<!-- agent:exchange -->` opener (producing two complete exchange blocks), the binary auto-repairs by merging content from the second block into the first and removing the second block. This repair runs in three places: (1) `repair_template_doc_if_needed` during preflight, (2) post-patch in `apply_patches_with_overrides`, and (3) explicitly via `agent-doc repair`. The merge is iterative — if more than two exchange blocks exist, the binary loops until only one remains. Content from later blocks is appended to the first block's content.
...
3. If no pending/captured repair path exists, repair still checks for stale completed backlog items:
  - for template docs, that no-pending path still runs the same safe transcript canonicalization used by the dedup path before backlog maintenance, including restoring missing `❯ ` prompt prefixes on already-answered prompt/response tails, repairing the safe duplicated `agent:exchange` close-marker shape, and repositioning a stale boundary to the real end of the completed turn when that repair is deterministic
...
- Prompt-bearing drift classification must also ignore an already-answered exchange tail that sits below a stale `agent:boundary` marker. If the current exchange tail's oldest prompt-bearing line is immediately followed later in that same tail by either a real `### Re:` / `## Assistant` response block or clear raw assistant completion prose (for example `I updated ...` plus follow-up bullets) and no newer prompt-bearing tail follows it, the shared `prompt_bearing_changes` classifier must suppress that answered run before `preflight`, `plan`, prompt-prefix normalization, write-path drift handling, `session-check`, or routed cycle-ack gating consume it; the remaining closeout handling is commit-boundary repair, not `pending prompt_target`.
- For live Codex reroutes, once the correct pane/file binding has been proven and the bare `agent-doc <FILE>` reopen has been accepted into that pane, route may degrade to optimistic success instead of fail-closing on missing later submission proof, missing cycle-ack, or the alive-pane busy-session retry ladder after cancel + `/clear`. Route must still record startup-miss diagnostics, and dead panes still fail closed.
...
If fresh-start recovery clears the new pane's early geometry-only registration while the harness is still booting, route must re-check and restore that exact pane binding immediately before the first guarded trigger injection. That self-heal is allowed only when the pane is still unbound or already bound to the same document. If startup hands the same session/document back to an already-running pane, route must follow that authoritative owner and dispatch there instead of continuing to target the temporary boot pane. Only true cross-file conflicts still fail closed; route must not silently steal a pane that is now registered for another file.
...
Editor-driven sync and manual `start` must apply the same supersession rule before any fresh-pane provisioning path. If the startup-miss marker is already stale because a newer registered owner is open, clear it first. If the marker still belongs to an alive registered pane, those entrypoints must fail closed or refocus the existing owner instead of creating another replacement pane and making a registry rebind look like a tmux-pane crash. More generally, `start` must not treat an alive registered pane with missing ownership proof plus an unavailable supervisor as permission to clear the binding and replace it, unless that pane is already stranded in a stash window and both the startup-miss + session-log provenance show the old owner is closed. A halted supervisor is still not sufficient proof to replace the pane when the session log shows that pane's latest run never recorded a child exit or `session_end`. When `start` does intentionally replace an alive pane after an explicit crashed/halted determination, it must preserve the old binding until the new pane registers so the normal `session_superseded` and `session_end origin=registry_rebind` provenance is appended to the session log.

### Live-child ack suppression

When route detects a live agent-doc child process for the target file in the resolved pane (via `find_live_owner_pane`), it still sends the routed trigger, but prompt-bearing drift on top of a closed cycle must still produce a real per-document cycle acknowledgment. A merely live child is not proof that the new routed prompt was consumed into a fresh cycle. Route therefore now treats cycle-state acknowledgment as a **post-dispatch health check**, not the first dispatch-start proof: for Codex, the tracked hook prompt must first prove that the bare reopen was consumed, and only then does route wait for the newer cycle state. Hook-backed dispatch-start proof is enabled when the rerouted file can actually reach a tracked `.codex/hooks.json` on its own upward `.codex` walk, not merely when some higher tracked `.agent-doc` ancestor has hooks on disk. That keeps workspace-level installs active for normal child-repo documents like `src/session-share/tasks/*.md`, but it stops demanding hook proof when a nearer nested `.codex` path shadows the ancestor install and the live Codex pane therefore cannot emit `UserPromptSubmit` state for that reroute. This guard only considers prompt-bearing drift from the document body; frontmatter-only metadata edits do not qualify, and neither does a stale-boundary tail whose oldest prompt is already visibly answered by a later `### Re:` / `## Assistant` block in that same tail. Because a healthy live owner can take longer than a fresh pane to surface the next `preflight_started` boundary after a routed prompt, this path uses a longer bounded acknowledgment window (30s in production, 2s under tests) before it records `route_cycle_start_missing`. If route first times out on the idle-prompt heuristic, it may run one scoped `agent-doc fix` pass; when that fix is a no-op, the same pane still authoritatively owns the document, and supervisor health is still `healthy`, route now performs one bounded **fresh** restart before a final reroute instead of first reinjecting the reopen into that still-busy pane. For Codex specifically, route also treats the latest tracked hook prompt `/clear` as a capability-reset marker: before the next reroute into that live pane, it asks the supervisor for a **fresh** restart so the original `codex_args`, writable roots, and network policy are reapplied instead of trusting post-clear resume inheritance. A later live-pane routed miss after an accepted/consumed dispatch still gets one deterministic fresh-restart recovery attempt before the final startup-miss path: route waits for the restarted Codex prompt to become dispatch-ready again, re-resolves the authoritative pane after any supervisor handoff, resends the same bare `agent-doc <FILE>` reopen once, and then still requires the later cycle-start health signal before success. If that replacement pane never becomes dispatch-ready, route must fail at the dispatch-readiness stage instead of recording a misleading "no new cycle started" startup miss. If the pre-dispatch busy-pane ladder instead falls through the bounded interrupt closeout, that final fail-closed error must preserve the last recovery-stage blocker rather than reusing the stale pre-interrupt timeout. The retry must preserve the same resolved absolute file path that the original dispatch used; it must not fall back to a caller-relative `file.display()` rendering after the restart handoff. If that one-shot fresh restart still produces no new cycle after dispatch-ready resend proof, route records the routed startup-miss and fails closed as before. The only suppression remains the open-cycle case: if the baseline cycle is already in flight, route does not require another start ack for that same pending work.
...
`repair_layout` normalizes the tmux window layout before every sync, and sync runs it again after tmux-router finishes mutating panes so the final window order is corrected as well. It receives the tmux handle, session name, and target window name (always `"agent-doc"`). When sync is windowless, the session still resolves through the shared precedence contract above before layout repair runs, so a live project `tmux_session` pin keeps sync scoped to that session's `agent-doc` / `stash` windows instead of inheriting an unrelated attached session.
...
**Phase 3 — Index normalization:** Re-lists windows after Phases 1+2, moves the `agent-doc` window to index 0, then moves any remaining stash windows directly after it (`1`, `2`, ...). If a destination slot is occupied, the repair path uses `swap-window` so unrelated user windows are preserved instead of being dropped. This phase always runs.
