...
- **Successful routed Codex turns keep their pane** — when a routed/editor-owned Codex child already committed a document cycle, the supervisor must restart fresh instead of entering the Ctrl-D quit prompt path so a successful turn cannot immediately close the claimed tmux pane.
...
- **Required SSH drift detection must catch bare socket EPERM when SSH context is proven** — when a document declares `required_ssh_targets`, treat resumed Codex `command_execution` output like `socket: Operation not permitted` as required-SSH capability drift when the same event proves an `ssh` command against one of those targets. Keep localhost/CDP `Operation not permitted` signatures on the separate browser-capability retry path.
- **Route progress diagnostics must stay UTF-8 safe** — when `agent-doc route` prints captured tmux status lines while waiting for readiness, truncation must stay on char boundaries so Unicode prompt/status glyphs cannot panic a live reroute.
...
- **Passive editor sync should use the fast handoff ordering** — for `agent-doc sync --no-autostart`, prefer the latest matching pane first, then an alive exclusive registered pane for the same document root, and only then cold-start a new pane. Leave the heavier supervisor/process-tree recovery logic to the non-happy-path cases instead of making every tab switch pay for it.
