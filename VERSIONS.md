...
- **JetBrains `Run Agent Doc` now gives rebooted Codex panes a 120-second dispatch-readiness window.** The editor action now invokes `agent-doc route --dispatch-only --plain-trigger --wait-for-ready 120`, matching the slow-start reboot shape observed on 2026-06-23 where the recovered pane became prompt-ready shortly after the previous 60-second route wait had already failed with `unblocker=wait_for_dispatch_ready_prompt`. The route still fails closed when no prompt-ready proof appears; it just no longer reports a premature startup timeout for panes that are still legitimately booting.
