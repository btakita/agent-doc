...
| `preflight`, queue maintenance | `agent-doc-document`, `agent-doc-element-queue`, `agent-doc-queue`, `agent-doc-turn`, `agent-doc-work-graph` | Preflight consumes parse/projection facts; it should not own queue semantics. Completed: queue item lifecycle moved to `agent-doc-element-queue`; id-backed queue directive target parsing, queue command/prompt classification, queue continuation drainability/noise/context-reset policy, and guidance wording moved to `agent-doc-queue`; pure Auto-DAG classification/rendering moved to `agent-doc-work-graph`; cycle phase transitions moved to `agent-doc-turn`; orchestration keeps command/preflight/sidecar adapters. |
