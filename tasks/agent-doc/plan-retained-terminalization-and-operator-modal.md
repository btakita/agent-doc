...
1. A captured closeout that has reached the exact canonical and disk target must terminalize exactly once regardless of whether delivery convergence or `session-check` observes equality first.
...
- Retained closeout: `agent-doc-closeout-runtime-io` owns capture settlement and commit; `agent-doc-session-check-io` must invoke it for every resumable phase before declaring terminal convergence.
- Pane routing: `agent-doc-harness` owns typed pane-blocker classification and `agent-doc-queue::route_dispatch` owns which blockers are queueable. `agent-doc-route-io` applies the queue decision without synthesizing pane input.
...
| Active harness turn | Queue behind owner |
...
- Cycle phase, capture id, response hash, retained target hash/content, canonical hash, disk hash, live-editor count, and delivery convergence.
...
- `agent-doc-route-io`: apply the queue-before-repair decision.
- `agent-doc-controller`: render the exact operator remedy at the edge.
- Version metadata and this architecture record.

## Verification

- Pure tests for equal-target capture resumption, artifact-picker classification, queue-source mapping, and recovery wording.
