...
| Controller RPC/CAS | `agent-doc-controller` | Active. Controller stores/apply decisions; it does not own supervisor policy. Pure dispatch admission helpers for in-flight coalescing, operator reopen classification, stale-generation redirect parsing, stale-queue pause recovery, recycle debounce, and force-recycle bypass now live in `agent-doc-controller`; orchestration imports them directly and the RPC layer remains only the socket/process adapter. |
