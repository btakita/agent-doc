...
- **Queued editor reruns preempt the auto-loop** — a dispatch-only editor rerun queued behind a busy actor in `agent:queue auto` is inserted **ahead of pending auto items** (priority preempt, `#jb-run-preempt-autoloop-priority`): a manual `Run Agent Doc` jumps the queue rather than landing at the tail. The priority insert lands after any leading queue directive (preset / start fence) and never supersedes a lone auto prompt, so the pending auto-loop item is **preserved** (the manual prompt runs first, then the pending item) instead of being replaced. If an explicit `Run Agent Doc` finds a busy authoritative actor and the document already has a startable inactive `agent:queue auto` head, route promotes that head to `queue_active: true`, syncs the snapshot, and lets the supervisor idle-queue watch drain it once the pane is idle instead of failing as a silent no-op.
