...
- **Busy live-pane reroutes now auto-apply the scoped fix path once before the final fail-closed error.** `route.rs` no longer surfaces the raw "not showing an idle prompt" failure on the first pass for a live same-document pane with unresolved prompt-bearing drift. Route now runs the same document-scoped repair path as `agent-doc fix <FILE>`, re-resolves the authoritative pane, and retries dispatch one time before failing closed; if that repair leaves the same pane authoritative and its supervisor is still reachable, route also performs a one-shot in-place supervisor restart before the retry. Added regression coverage for both the bounded fail-closed message and the successful one-shot busy-pane recovery path, and updated the command spec. This closes the latest JetBrains `Run Agent Doc` repro from `tasks/agent-doc/agent-doc-bugs2.md`.
