...
- When Codex hook tracking is visible for a dispatch-only reroute, a live-pane or supervisor-backed bare reopen should record routed submission proof when the hook state advances inside the bounded wait. If the command leaves the input surface but Codex never records that proof, route still treats the shared text+`Enter` acceptance as successful delivery and logs `proof=accepted proof_scope=accepted_only` so log review can distinguish accepted delivery from stronger dispatch-start proof.
...
- Dispatch-only OpenCode reroutes must not report routed delivery as successful on pane-input acceptance alone. OpenCode proof requires the routed trigger to leave the composer and the pane to leave idle chrome within the OpenCode redraw budget. Codex and Claude Code dispatch-only routes may use accepted-only delivery; Codex still records stronger hook proof when it appears.
...
- For dispatch-only Codex reroutes with visible hook tracking, accepted tmux delivery without routed submission proof is a successful accepted-only delivery. Hook proof upgrades the log scope to `dispatch_start`; missing hook proof does not make the editor action fail.
...
- Claude Code and Codex dispatch-only routes label accepted pane delivery as accepted-only when no dispatch-start proof is available. OpenCode can satisfy its proof requirement only through pane-state transition proof (`proof=pane_state_changed proof_scope=dispatch_start`).
