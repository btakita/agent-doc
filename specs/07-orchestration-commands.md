...
- While a CRDT streaming step is still generating, partial output may be checkpointed only through the shared recovery-sidecar ledger so pane death does not discard all in-progress assistant text. The finalizer always receives the complete response, never a suffix computed from a document-visible prefix.
