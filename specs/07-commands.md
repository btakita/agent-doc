...
**Pending/backlog patch normalization:** Agents must still use granular `--pending-*` flags in normal response cycles, but the binary now has a compatibility path for a single list-shaped `replace:pending` / `patch:pending` block that slips into the response anyway. Before durable capture, `write` / `finalize` / `stream` / `ipc` parse that block, rewrite the live pending component through the same granular pending primitives, drop the pending patch from the captured response body, and only then persist the response. Unsupported shapes (multiple backlog patches, non-list mutations, or pending prelude/postlude edits) still fail closed **before** `response_captured` so the cycle is not stranded by a forbidden patch after capture.
...
- Pending-done guard: after a committed cycle, `session-check` also inspects the committed response capture against still-open `agent:backlog`/`agent:pending` ids and warns when the response appears to complete an existing `#id` task but the cycle recorded no matching `--pending-done <id>`
