...
- The tracked event families are `ipc_write_consumed`, `commit_success`, `commit_noop`, `route_dispatch_start_proven`, `post_commit_user_follow_up`, `post_commit_local_drift`, `session_clear_active_pane_allowed`, `session_clear_protected_input_guard_refused`, legacy `session_clear_live_busy_guard_bypassed` / `session_clear_live_busy_guard_refused`, `route_authoritative_actor_starting_not_ready`, dispatch-only route lines with `proof_scope=accepted_only`, and `sync_latency` entries with `status=over_budget`.
