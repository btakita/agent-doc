...
- **Selective commit is conservative** — `git::commit()` stages the snapshot, not arbitrary working-tree drift. The only absorbable out-of-band repair path is narrow agent-owned drift (`status`, appended `### Re:` response, pending-ID superset) when the redacted component structure still matches. Plain user prompts must remain uncommitted. After a successful commit, boundary cleanup must normalize the snapshot and working tree back to a clean single-boundary shape with no transient `(HEAD)` markers; only skip the direct file rewrite when a live IPC listener is available to perform that cleanup in the editor.
