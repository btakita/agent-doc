...
- File-scoped `agent-doc session clear <FILE>` must not run the starting-actor readiness guard and must not fail solely because the resolved pane's `pane_current_command` is the `agent-doc` wrapper. Clear is an explicit operator context-reset request: direct `alive-busy` evidence may log that the active pane was allowed, but the command still proceeds through the normal `/clear` submit path unless the captured pane contains protected prompt input such as a permission prompt, queued draft, shell history search, or drafted user text. Operators can use `agent-doc session interrupt-clear <FILE>` for intentional discard of those protected states.
