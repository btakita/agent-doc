...
- **Retained ACK replay:** After a remote target is visible, the plugin retains every delivery ACK until the controller accepts the current editor-content hash. Failed ACK transport or a refused hash is retried idempotently from the current buffer on later drains. A replace re-bootstrap retires the old member's ACK ledger before registering the new lineage. The `ack_recovery_force_refresh` event force-refreshes only the named open document from its editor/Lazily reconnect target.
