...
- File-scoped `agent-doc session clear <FILE>` must not run the starting-actor readiness guard. Clear is an explicit operator context-reset request, but it must still refuse an alive-busy `agent-doc` pane until that pane shows an idle dispatch-ready composer or a clean-exit restart prompt. This prevents Clear Session Context from sending `/clear` into an active agent turn, shell history search, or similar busy state; operators can use `agent-doc session interrupt-clear <FILE>` for intentional interruption.
