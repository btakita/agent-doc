...
- **Auto-update prompt (`#codex-skill-reload-cpc-restart`):** after `agent-doc skill install --harness codex --reload restart` installs a real update, immediately run `agent-doc session restart-supervisor <FILE>` and stop. The CPC accepts the owner-pane handoff, restarts Codex in continue mode (`codex resume --last`), and automatically re-submits the document trigger with the updated skill. Do not ask the user to restart when the CPC handoff succeeds; manual restart guidance is only the fallback when the controller reports that no managed session can be restarted. If install says the skill is already up to date, continue the current turn without restarting.
