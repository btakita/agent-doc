...
5. If `sessions.json` points at a different **alive** pane but no live owner can still be proven for the document, consult the supervisor before treating it as stale:
   ...
   - **Halted** (supervisor state is `halted` after repeated crashes) → do not revive that supervisor in place; clear the crashed registration and start fresh in the current pane
   - **Unreachable** or **No socket** → if that same alive pane still owns the active startup-miss marker for the document, fail closed instead of rebinding to a fresh pane; otherwise clear the stale registration and continue in the current pane
