...
That reload broadcast updates only the native cdylib already hosted by an installed editor plugin. It cannot update the JetBrains Kotlin plugin, the VS Code TypeScript extension, or their reported package versions. When `stale_plugin` reports an older plugin version, install/update the plugin package and restart/reload the IDE host. At every replay/ACK boundary, use the latest live registration; an expected-or-newer registration after a mid-turn restart supersedes the earlier preflight warning. If a new console prompt arrives while the older closeout is pending, persist its exact text first with `agent-doc exchange add-prompt <FILE>`; the authority-aware mutation preserves/rebases the active capture. Then resume once with `agent-doc write --commit <FILE>` rather than postponing the prompt, leaving it only in transcript memory, or alternating finalize/write retries.
