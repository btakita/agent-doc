...
- `agent_doc_queue_context_reset = <bool>` — explicit opt-in for the supervisor idle-queue watch to pre-emptively interleave a context-clear (`/clear`/`/new`) before an active queue head when session accretion is unhealthy (`#nm1x-no-preempt-clear`). Default off. When absent, a manual `Run Agent Doc` or an auto-loop drain never fires a pre-emptive `/clear`, so the harness pane is not churned and `/clear` is never sent into an in-progress turn. A document-frontmatter `agent_doc_queue_context_reset: true` takes precedence over this project setting. Deferred *operator* clears (an explicit `session clear`/`interrupt-clear` that was deferred past a busy turn) are a separate path and remain live regardless of this opt-in.
