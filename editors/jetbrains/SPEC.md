...
- `Run Agent Doc` saves only the active markdown document and immediately dispatches `agent-doc route --dispatch-only --plain-trigger --debounce 0 --wait-for-ready 15`. The editor path does not block on the typing debounce and does not save unrelated open documents; the active document save is the editor-owned flush boundary before the binary route command runs. Even after `Clear Session Context`, this action still sends the plain `agent-doc <FILE>` reopen into the live session instead of restarting Codex.
- Every `Run Agent Doc` click writes a durable attempt ledger entry for click receipt, active document save, route command construction/start, retry/supersede, and terminal route outcome. The ledger may persist route command text and route output summaries, but it must not persist raw document prompt text; prompt/trigger proof is represented by byte counts and hashes in binary/controller diagnostics.
- Repeating `Run Agent Doc` while an older plugin-spawned route process is still alive supersedes that stale route process and starts the latest dispatch immediately. The superseded attempt is recorded in the durable ledger so the newest click owns the plugin route process.
- `Run Agent Doc` and `Clear Session Context` are serialized per document in the JetBrains action layer before either command reaches the binary. Repeated Run clicks replace the first alive route process; `Clear Session Context` preempts a still-dispatching Run by canceling the plugin-spawned route process and then running the normal binary-owned `agent-doc session clear <relative-path>` path. If Run is clicked while a normal clear command is already running, the latest Run intent is queued and starts only after the clear completes synchronously. A clear accepted for deferred delivery by the binary does not release the queued Run immediately.
