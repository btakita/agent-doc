...
- `agent_doc_queue_context_reset = <bool>` — explicit opt-in for any accretion-driven pre-emptive context-clear (`/clear`/`/new`) before an active queue head when session accretion is unhealthy (`#nm1x-no-preempt-clear`, `#nm1x-codex-clear-parity`). Default off, and product-wide: it gates the supervisor idle-queue watch, the Codex Stop-hook continuation instructions, and the `run.rs` queue-continuation fresh-agent-session decision alike. When absent, a manual `Run Agent Doc` / Codex Stop-hook continuation / auto-loop drain never fires a pre-emptive `/clear`, so the harness pane is not churned and `/clear` is never sent into an in-progress turn. A document-frontmatter `agent_doc_queue_context_reset: true` takes precedence over this project setting. Deferred *operator* clears (an explicit `session clear`/`interrupt-clear` that was deferred past a busy turn) are a separate path and remain live regardless of this opt-in.
- `agent_doc_clear_threshold = <0..100>` — context-usage percentage at or above which an opted-in editor (JB `Run Agent Doc`) pre-emptively runs `/clear` before the next dispatch (`#clear-opt-in-threshold`). Resolution: per-document frontmatter `agent_doc_clear_threshold`, then this project setting, then the built-in default of `50`; values are clamped to `0..=100`. The binary owns the threshold value so every editor shares it and surfaces it on the preflight `session_accretion.clear_threshold` field (`session_accretion::clear_threshold_for_doc`); the live context-percentage comparison and the actual pre-emptive clear are the editor's responsibility (gated on live-editor verification). This is the configurable companion to `agent_doc_queue_context_reset` (the on/off opt-in).
- `agent_doc_supervisor_auto_recycle = <bool>` — explicit opt-in/opt-out for the route-owned supervisor to `execve` hot-reload onto a freshly-installed binary at an idle / inter-queue-item boundary (`#ctlrecycle` R3 / `#suprecyclequeue`), preserving the live agent child + pane. Resolution: the env var `AGENT_DOC_SUPERVISOR_AUTO_RECYCLE` (truthy `1`/`true`/`yes`/`on` force-enables, falsey `0`/`false`/`no`/`off` force-disables), then a per-document frontmatter `agent_doc_supervisor_auto_recycle`, then this project setting, then the built-in default of OFF. When off everywhere, a stale supervisor only logs `supervisor_binary_stale_detected` until a project, document, or operator opts in (the image swap is high blast-radius; dog-fooding repos that rebuild constantly set this so the supervisor always promotes the latest build automatically). Resolved by `resolve_supervisor_auto_recycle` / `supervisor_auto_recycle_enabled`.
