...
- If manual `Sync Tmux Layout` exits `0` but reports that it preserved the current layout because a visible protected pane could not detach yet, JetBrains must show that preserve-layout warning visibly instead of only logging it.
