...
- Protected-busy clear refusals must show a typed warning with `Refresh and retry`, `Interrupt and clear`, `Show status`, and `Copy details` actions instead of the generic command-failed text.
- `Refresh and retry` first runs `agent-doc session status <relative-path>`; if the returned `live_pane:` line is already `state=alive-idle` with `prompt_ready=true`, the plugin immediately retries `agent-doc session clear <relative-path>`, otherwise it shows the exact status output, including actor/supervisor/controller projection and direct live-pane evidence, and leaves `Clear Session Context` available as the non-destructive operator recovery action.
- `Interrupt and clear` requires an explicit IDE confirmation and then runs `agent-doc session interrupt-clear <relative-path>`, leaving harness-specific interrupt keys, idle/closed waiting, and the final clear retry in the binary-owned operator path.
