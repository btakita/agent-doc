...
- **Editor Clear Session Context busy-pane UX is now explicit across plugins.** JetBrains keeps the binary-owned protected-busy guard and the typed Refresh/Interrupt/Status/Copy warning, and the shared editor specs now state that direct `alive-busy` evidence fails closed while stale busy projection is reconciled only after direct `alive-idle prompt_ready=true` proof. VS Code now mirrors the same busy-refusal parser and operator actions, including `agent-doc session interrupt-clear <FILE>` behind an explicit confirmation. Bumped local plugin builds to JetBrains `0.2.119` and VS Code `0.2.18`. This closes `#jbclearstaleidle` in `tasks/agent-doc/agent-doc-bugs2.md`.

- **Clear Session Context now refuses live-busy panes unless idle proof is explicit.** File-scoped `agent-doc session clear <FILE>` still reconciles stale busy projections when direct Codex/OpenCode idle evidence is present, but a pane classified `alive-busy` now fails closed before `/clear` is submitted. The refusal records `session_clear_live_busy_guard_refused` and points operators to `agent-doc session status <FILE>` for an idle-proof retry or `agent-doc session interrupt-clear <FILE>` for an intentional interrupt-and-clear discard. This closes `#q97c` in `tasks/agent-doc/agent-doc-bugs2.md`.
