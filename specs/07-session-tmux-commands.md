...
- If a prompt-bearing dispatch-only reroute reaches a live pane that reports a plain active Codex or OpenCode turn before input is submitted, route must queue that rerun in `agent:queue auto`, sync the snapshot, and print a `queued pending dispatch` diagnostic instead of surfacing a persistent editor route failure. This queue path is only for active-turn blockers; hook review prompts, queued drafts, permission prompts, shell history search, and clean-exit restart prompts still fail closed with their specific recovery action.
