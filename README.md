...
- **Successful routed Codex turns keep their pane attached** — if a routed/editor-owned Codex child already committed a document cycle, a forwarded `Ctrl-D`/stdin EOF no longer drops the supervisor into the quit path and tear downs the claimed tmux pane; agent-doc restarts fresh instead
