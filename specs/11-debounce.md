...
Route startup treats an attached document with a retained delivery frontier as active backpressure. On the first `delivery_pending` observation it signals one urgent retained-delivery drain to the live editor replicas, then continues polling the canonical convergence state. It must not rely on an editor's ordinary background retry timer to fire within the route deadline, and it still fails closed if convergence does not complete before that deadline.

Preflight uses the same fail-closed settle contract, with one final authority observation at the defer boundary. This prevents an editor acknowledgement that lands between the last ordinary sample and the deadline decision from being reported as a stale `delivery_pending` failure. Convergence succeeds immediately; newly visible progress resets only the no-progress deadline; a still-pending observation or the absolute progress ceiling continues to defer.
