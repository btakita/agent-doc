...
- The Codex install surface also writes repo-local `.codex/hooks.json` plus `.codex/config.toml` with `features.codex_hooks = true`; the `UserPromptSubmit` / `Stop` bridge must track the active document, capture `last_assistant_message` into the normal pending/capture ledger when the stop arrives with an open cycle, auto-continue once, and then fail closed if the cycle still is not resolved.
