...
**Compact full-content patches:** `agent-doc compact <file> --component exchange --commit` uses the guarded direct-write path rather than emitting editor IPC `fullContent`. Editor plugins still defensively validate legacy/foreign `fullContent` payloads, but first-party Compact Exchange must not depend on whole-document editor replacement.
