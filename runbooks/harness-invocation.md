...
- **Auto-queue heading discipline (`#codex-queue-head-exact-heading-guidance`):** when a Codex auto-loop turn completes the active `agent:queue auto` head, the Stop-hook auto-consume fires only on an explicit completion signal. Either (a) pass `--done <id>` (or `--pending-gate <id>` / `--pending-edit "<id>=…"`) on `agent-doc finalize` naming the head id, or (b) title the response heading **exactly** with the queue head — `### Re: do [#id]` for a `do [#id]` head, or `### Re: #id` for a preset/synthetic head (a prompt carrying a trailing `#preset` id). A descriptive heading (`### Re: <free-form topic>`) does **not** auto-consume the head, so the auto-queue will not advance and the hook fails closed on the next non-advancing repeat. Headings whose topic carries trailing modifiers (`### Re: #id halt` / `### Re: #id deferred`) never consume — that is the intended halt/refusal behavior (`#queue-strike-on-halt`).
