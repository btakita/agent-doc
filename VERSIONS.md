...
- **Post-exchange hidden prompt duplicates are cleaned during closeout.** Final template reconciliation now removes ordinary HTML comments below `agent:exchange` only when the comment body duplicates or near-duplicates the unresolved prompt being answered, covering the observed `agent-doc-bugs2` prompt-copy residue while preserving unrelated scratch comments outside the exchange. Added focused write-path regressions and updated the closeout spec.
