...
- **FlowCore closeout mirror:** Closeout command paths must be describable as FlowCore stages: pre-write guard, patchback parse, document mutation, snapshot convergence, pre-commit guard, commit, terminal guard, and session-check. The implementation emits mirror-mode `flow_event` records for commit completion, document-mutation parse outcomes, strict pending-capture / pending-done guard blocks, repair recovery boundaries, and committed-cycle late-fallback rejection. Future closeout fixes should extend those typed events instead of introducing independent proof strings.
...
- Template patchback shape classification belongs to `document_mutation`. Patch markers with no closed patch blocks are `malformed_patch`; patch blocks plus unmatched output are `mixed_output`; transcript-shaped dumps are not valid template patchbacks unless a command explicitly owns a narrow normalization path.
