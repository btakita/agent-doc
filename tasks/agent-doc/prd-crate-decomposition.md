...
| Turn lifecycle, operation log, and turn-scope affectedness | `agent-doc-turn` | Active. Lifecycle consumes realtime handoff proof and owns commit policy. Cycle phase transitions, closeout response/done-signal parsing, reaped queue-directive response-loss detection, free-text queue response-proof/residue detection, blocked follow-up closeout detection, gated-phase split body detection, queue-audit partial-completion collapse detection, operation-log data types, turn-scope manifests, affectedness classification, and queue-continuation stall classification live here; orchestration imports `CyclePhase`, closeout signal policy, and drain-stall policy directly from `agent_doc_turn` and owns only sidecar persistence and command adapters. |
...
| `heuristics.rs`, closeout response/done-signal parsing | `agent-doc-turn` | Extracted. Pending-capture recommendation detection, explicit no-follow-up response detection, closeout response/done-signal parsing, reaped queue-directive response-loss detection, free-text queue response-proof/residue detection, blocked follow-up closeout detection, gated-phase split body detection, and queue-audit partial-completion collapse detection are turn closeout policy; orchestration only applies the resulting signal to guards and file/cycle adapters. |
...
| `session_check`, closeout recovery classification | `agent-doc-turn` plus document realtime projections | Session check consumes lifecycle/realtime facts; it should not re-own source authority. Queue-continuation stall classification, closeout response/done-signal parsing, reaped queue-directive response-loss detection, free-text queue response-proof/residue detection, blocked follow-up closeout detection, gated-phase split body detection, and queue-audit partial-completion collapse detection are now in `agent-doc-turn`; orchestration persists the one-shot marker and adapts file/cycle state. |
