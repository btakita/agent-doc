...
- Generated on first insert (via `--pending-add` or lazy backfill).
...
| Flag | Behavior |
|------|----------|
| `--pending-add "text"` | Append new item. Binary assigns hash and `[ ]`. |
...
- Do we want a `pending-done` archive component, or is git history sufficient? Defer — no one has asked for this yet.
