...
- **Starting actor reroutes wait or fail closed before input** — when the controller still reports the authoritative actor as `starting`, route waits briefly for the actor to refresh to `ready`; if it stays `starting`, both managed and dispatch-only reroutes refuse to send tmux or supervisor input instead of injecting into the startup window
