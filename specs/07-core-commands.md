...
- `#clearcodex`: the Codex Stop-hook continuation clear decision must be observable in `.agent-doc/logs/ops.log`, mirroring the supervisor idle-queue watch (`#s760c`). Whenever the project is opted into `agent_doc_queue_context_reset`, each Codex continuation emits the canonical `[s760] clear-decision optIn=… threshold=… pct=… clear=…` line plus a `[clearcodex] codex-continuation optIn=true reason=… clear_instructed=…` companion that records the accretion/compaction reason requiring fresh context. When that reason is present, the hook also emits `codex_fresh_context_handoff ... result=queued supervisor=idle_queue_watch` and returns control to the supervisor instead of instructing the operator to run `/clear`. Codex transcript token reading is unsupported (`context_pct::read_used_tokens` → `None` for Codex), so the ctx% gate always logs `pct=none clear=false` and never fires the clear on its own — the accretion/compaction reason is the live signal. When the project is not opted in, the continuation stays silent (no marker, no pre-emptive clear). This gives the operator a structured log proof to confirm or deny a queue-turn clear without re-deriving it. The `s760_clear_decision_clear_true` gate verifier accepts only a real anchored `^[epoch] [s760] clear-decision ...` line with `optIn=true`, `pct >= threshold`, and `clear=true`; quoted prose in queue-diff logs is not proof.
