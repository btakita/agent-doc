...
- `#queue-context-reset`: after a clean active queue item closeout, an automatic continuation must inspect session accretion before launching the next prompt. If the document is at `warn`/`block` accretion or the exchange was recently compacted without a later tracked context clear, direct `run` must start the next backend call from a fresh agent session (ignore the current `resume` for that dispatch, then persist the returned new session id). In Codex Stop-hook continuation, the hook must not ask the already-loaded pane to answer the next queue head and must not punt `/clear` to the operator; it records the requested head, logs `codex_fresh_context_handoff`, allows the turn to close, and lets the managed supervisor idle-watch perform the clear/settle/dispatch sequence. A recent exchange compaction always requires at least one fresh context/clear before a many-turn queue continues in-pane.
- `#clearcodex`: the Codex Stop-hook continuation clear decision must be observable in `.agent-doc/logs/ops.log`. Whenever the project is opted into `agent_doc_queue_context_reset`, each Codex continuation emits the canonical `[s760] clear-decision optIn=… threshold=… pct=… clear=…` line plus a `[clearcodex] codex-continuation optIn=true reason=… clear_instructed=…` companion that records the effective reset reason. Codex context percentage is read from the latest matching `~/.codex/sessions/**/rollout-*.jsonl` `token_count` event (`last_token_usage` plus `model_context_window`); if no readable event exists, the ctx% gate logs `pct=none clear=false` and fails safe. When the numeric Codex pct crosses the configured threshold, or when accretion/compaction requires fresh context, the hook emits `codex_fresh_context_handoff ... result=queued supervisor=idle_queue_watch` and returns control to the supervisor instead of instructing the operator to run `/clear`. The supervisor idle-queue watch then re-resolves the same opted-in reset reason at the safe idle boundary; if still active, it sends `/clear`, waits for settle, and only then dispatches the ordinary queue head. Route-in-flight, active-turn, pending-clear, and one-clear-per-head gates remain mandatory. When the project is not opted in, the continuation stays silent (no marker, no pre-emptive clear). This gives the operator structured hook-path log proof to confirm or deny a queue-turn clear without re-deriving it. The `s760_clear_decision_clear_true` gate verifier accepts only a real anchored `^[epoch] [s760] clear-decision ...` line with `optIn=true`, `pct >= threshold`, and `clear=true`; quoted prose in queue-diff logs is not proof.
