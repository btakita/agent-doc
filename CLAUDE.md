...
- **Codex hook backstop is binary-owned** — keep `src/codex_hook.rs`, `src/skill.rs`, `SKILL.md`, `runbooks/harness-invocation.md`, `README.md`, and `SPEC.md` aligned on the installed `.codex/hooks.json` / `.codex/config.toml` contract: `UserPromptSubmit` tracks the active document, `Stop` captures `last_assistant_message` into the pending/capture ledger when a cycle is still open, auto-continues once, then fails closed.
