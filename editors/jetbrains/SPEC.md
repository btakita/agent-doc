...
- Split-layout tab-selection changes submit `agent-doc.sync_tmux_layout.v1` with `no_autostart=true` through the Project Controller command plane so a selected visible document can be rescued back out of stash into the `agent-doc` window without replacing a live owner. That passive path hands off quickly by preferring the matching pane and then an alive exclusive registered pane; the preceding typed focus command owns resume of a missing/dead actor.
...
- Dedup state tracks the visible markdown signature plus the active file so repeated selection churn does not rerun the same command, but a real markdown `selectionChanged` event still submits the coalesced focus handoff even when the layout signature is unchanged. The focus payload uses the closed-set `missing_pane_policy=resume_latest` contract, so selecting a document whose actor pane died resumes the latest document session before focusing it; older payloads default to `observe_only`.
...
- On a real markdown selection event, JetBrains starts a best-effort immediate Project Controller command-plane `focus_document_pane` handoff for the event's selected document before debounce. A closed/missing actor projection may yield to the latest open session-log pane only when that pane still runs a live agent and its process tree exactly owns the selected document; bare-shell and cross-document reuse fail closed. With `resume_latest`, a missing/dead owned pane is restarted through the controller before focus; inactive-window receipts remain diagnostics. The guarded command-plane `sync_tmux_layout` reconciliation remains the authoritative background layout repair.
...
- The structural layout-change detector only reports a new editor surface observation into `EditorTabSyncListener`; it has no second CLI planner, lock, or tmux process. Tab selection, focus, and structural changes therefore share one latest-wins surface graph, preventing competing reconcilers from transiently provisioning three tmux panes for a two-editor split.
...
- JetBrains handles socket `save_document` messages as typed editor-owned saves. The handler waits for typing idle, locates the open IntelliJ `Document`, calls `FileDocumentManager.saveDocument(document)`, publishes the saved text through `agent_doc_editor_content_applied_for_editor_v1` with `lazily_transport_receipts_v1` for the supplied `patch_id`, and records its outcome through the shared `agent_doc_record_editor_surface_event` FFI schema. It must not format or append editor-surface ops-log events in Kotlin, write `.agent-doc/ack-content`, replace document text, use VFS binary writes, or revive reconnect reread repair on this path. Missing lazily receipt support is an incompatible plugin/native-library version error.
- Forced replica refresh registers the exact live editor cut, reports reconnect propagation, and schedules an ordinary remote drain. It must not synchronously reconstruct a retained response or mutate/save the editor through JNA during registration; retained replay is controller-owned CRDT delivery. This keeps registration bounded and prevents one document's recovery from disabling the shared native generation.
...
- Retained delivery acknowledgement replay is a cumulative frontier operation:
  ...
  through the newest pending target with that hash. A successful forced replica
  swap retires the old member's entire ACK frontier before its deregistration;
  historical generations must never monopolize the shared replica worker or
  leak onto the replacement identity.
- A remote CRDT delivery is acknowledged only after its IntelliJ `Document` is cleanly saved. Before mutating a clean document, the plugin refreshes that target file's VFS stamp and rechecks that the document stayed clean; an unsaved document fails closed into exact-editor-baseline retry. The save is permitted when a raw disk read still equals either the guarded editor baseline or the converged target; novel external disk text rejects the delivery without mutating or overwriting it. VCS-refresh signals mark the dirty scope without recursively refreshing project content behind unsaved editors. This keeps non-operator replica churn and stale VFS modification stamps from triggering IntelliJ's File Cache Conflict resolver while preserving fail-closed behavior for real external edits.
