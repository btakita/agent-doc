...
| Supervisor model and process effects | `agent-doc-supervisor`, `agent-doc-supervisor-process` | Active. Pure state/recovery decisions, supervisor config precedence, process lifecycle decisions, harness-change restart policy, child crash/restart policy, post-child-exit run-loop dispatch, and controller handoff state now live in `agent-doc-supervisor`; process handoff state for `execve` re-entry now lives in `agent-doc-supervisor-process`. The old `start::decisions`, `supervisor::state`, and `supervisor::resize` facades are deleted; orchestration imports the focused crates directly. |
