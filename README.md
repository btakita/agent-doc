...
- **Accretion guard** — `agent-doc plan` now blocks another normal implementation turn when the same document has clearly accreted too much exchange or restart churn; successful exchange compaction records a per-document recovery marker so the next turn measures closeout churn only after that compact instead of getting trapped in a compact-then-block loop, and remaining blockers still tell the caller to compact the exchange and/or restart from the current committed boundary first
