...
- **FlowCore closeout mirror:** Closeout command paths must be describable as FlowCore stages: patchback parse, document mutation, snapshot convergence, commit, and session-check. The current implementation emits mirror-mode `flow_event` records for commit completion and document-mutation parse failures; future closeout fixes should extend those typed events instead of introducing independent proof strings.
