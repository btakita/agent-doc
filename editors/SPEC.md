...
- **Feedback:** Inline hint near cursor for applied sync. If manual sync exits `0` but reports preserve-layout output, show that warning visibly instead of replacing it with a generic success hint.
...
- **Deferred preserve-layout handling:** If `agent-doc sync --no-autostart` exits successfully but reports that it preserved the current tmux layout because a visible protected pane could not be detached yet, the plugin must not mark the selection as synchronized unless the same output includes `[sync] safe_passive_layout_preserved_reselected_focus`. Keep the target pending and schedule bounded automatic retries until the passive sync applies or a newer selection supersedes it; this applies to both JetBrains and VS Code tab-sync implementations.
