...
**Column memory:** `.agent-doc/last_layout.json` persists a column→agent-doc mapping across syncs. When a column has no agent doc (user switches to a non-session file), sync substitutes the last known agent doc for that column index. Empty `--col` placeholders are preserved long enough for that substitution, so a right-hand markdown file can still restore a remembered left-hand pane instead of collapsing to one tmux pane. The state file is written after each successful sync for columns that contain an agent doc. The round-trip contract is: `apply_column_memory` restores empty placeholders from the persisted layout → tmux-router reconciles the expanded columns → `build_layout_state` records the new state → next sync cycle's `apply_column_memory` uses that state. Cross-root documents (e.g., `src/boost-client/tasks/monsterrodholders.md` alongside a root-level session) participate in column memory the same way: the canonical path stored in `last_layout.json` resolves through the same `first_agent_doc_in_col` session-UUID check regardless of which project root the file belongs to.
