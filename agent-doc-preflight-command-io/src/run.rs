    ...
    // clears it so the diagnostic fires once per stall, not every preflight.
    {
            ...
            .is_some()
        {
            ...
            let facts = StallFacts {
                ...
                // `#queuestallloopfalsepositive`: the STALL grace, not the
                // ownership TTL. A healthy self-paced cycle outlives the 90s
                // ownership window, so asking `fresh_loop_drain_owner_lease` here
                // reported every normal drain as a stall.
                loop_is_continuing:
                    agent_doc_queue_io::drain_owner::loop_drain_owner_lease_within_stall_grace(
                        &file_str, now,
                    )
                    .is_some(),
            };
