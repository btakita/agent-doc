...
- A prompt-bearing direct-pane reroute requires a new document-cycle acknowledgment, not merely accepted tmux input. If a long-lived Claude or Codex process predates the installed hook settings, route performs one fresh supervisor restart and retries so the harness reloads its startup hook snapshot; a second missing acknowledgment fails closed.
