...
- **operator** pin (top tier) — markdown **strong** emphasis on `pin`/`prioritized` (`**pin**`, `__pin__`, `**prioritized**`, `__prioritized__`), the `:pin:` / `:pushpin:` shortcodes, or the literal 📌 emoji.
- **agent** pin (middle tier, above unpinned, never above operator pins) — markdown *italic* emphasis on `pin`/`prioritized` (`*pin*`, `_pin_`, `*prioritized*`, `_prioritized_`), the `:round_pushpin:` shortcode, or the literal 📍 emoji.
