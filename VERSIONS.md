...
- **`agent-doc orchestrate` adds a first-class sequential orchestration surface.** The CLI now resolves task batches from repeated `--task`, `--from-file`, and `--from-exchange` sources under one `orchestrate --mode ...` command. `--mode sequential` injects each task into `agent:exchange`, runs `preflight`, sends one fresh agent request with no resume/session reuse, then persists each step through `finalize` followed by `session-check`; `--mode parallel` reuses the existing fan-out engine and `dag` is reserved with an explicit not-implemented error. Added unit coverage for task extraction, exchange injection ordering, fresh-session dispatch, and the sequential finalize path.
