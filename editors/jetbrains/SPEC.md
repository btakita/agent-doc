...
- If route fails only because the authoritative actor is still in its startup window, or because the latest dispatch-only Codex run is transiently booting/active, `Run Agent Doc` retries the same dispatch up to three times with bounded backoff before surfacing the final route failure. Protected-input blockers such as shell history search are not retried. Any later `Run Agent Doc` click still cancels the older retry loop and starts a fresh dispatch immediately.
- If route succeeds by queueing a prompt behind an already-busy authoritative actor instead of injecting a duplicate trigger, `Run Agent Doc` must surface a visible queued/still-running warning rather than treating the route process as silent success. Repeating `Run Agent Doc` after editing that same prompt must replace the sole live route-owned `agent:queue auto` prompt instead of leaving stale wording queued behind the active turn.
