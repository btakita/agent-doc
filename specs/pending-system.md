...
**Two pin tiers (`#queue-agent-vs-operator-pin-tier`):** operator priority always outranks agent priority, so there are two marker tiers:

- `__prioritized__` (double underscore) — **operator** pin. Top tier.
- `_prioritized_` (single underscore) — **agent** pin, for items the agent prioritized. Middle tier: above unpinned items, never above operator pins.

The sort key is `(tier, rank)` with `tier` ∈ {0 operator, 1 agent, 2 unpinned}; rank only orders the unpinned tail. The agent may add `_prioritized_` to items it prioritizes; the operator can override by deleting the agent marker or adding their own `__prioritized__`. Open question deferred by the operator: whether an agent pin may be placed *above* an operator pin — for now, no. Marker constants + detectors: `queue::PRIORITIZED_MARKER` / `queue::AGENT_PRIORITIZED_MARKER`, `queue::is_prioritized` / `queue::is_agent_prioritized`.
