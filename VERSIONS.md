...
- **Git-backed response persistence now owns the closeout `session-check`.** `agent-doc finalize`, `agent-doc write --commit`, and `agent-doc repair` now run the same post-commit `session-check` guard internally before they report success, so Codex/manual repair flows can no longer skip the terminal-cycle guard just because the harness forgot the extra command. `write --commit` remains best-effort at the raw `git::commit()` step, but for git-backed docs it now fails closed when the post-closeout guard still sees an open cycle or a bypassed patchback. Added regression coverage for strict pending-capture failures flowing through both `finalize` and `write --commit`.
