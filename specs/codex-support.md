...
3. ✅ Refactored `start.rs::run()`:
   ...
   - A supervisor restart that re-resolves to a different harness is a fresh launch, even though it occurs in the replacement loop: it skips same-harness exact-resume admission and automatically re-submits the owning `agent-doc` command once the new harness reaches its prompt
   ...
   - Fresh Codex startup now requires the reactive admission projection after trigger injection: route does not treat pane input acceptance as success by itself; it awaits the Project Controller's post-dispatch document-cycle projection and logs `fresh_route_start_projected` or `fresh_route_start_missing`
