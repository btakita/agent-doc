...
- **Exchange→queue prompt intake:** when `agent:exchange` carries the `queue` attribute, preflight moves newly-added exchange prompt blocks into `agent:queue` using the same `append` / `prepend` / `sync` modes, removes those blocks from the exchange tail, and suppresses the moved prompt diff for the current turn. This lets an operator add follow-up prompts while a queue is running without interrupting the current queue head or leaving unanswered direct exchange prompts behind.
