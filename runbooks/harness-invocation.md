...
- **`#preflightinbinary`:** the Claude `UserPromptSubmit` hook (`agent-doc hook preflight-user-prompt-submit`, installed into `.claude/settings.json`) and the Codex hook (`agent-doc hook codex-user-prompt-submit`, installed into `.codex/hooks.json`) run preflight in-process when the `agent-doc <FILE>` trigger arrives. They print the contract followed by the `[agent-doc] cycle contract ...` success marker; connected Codex controller flows may instead use `agent_doc_admit`. The model must never run or poll `agent-doc preflight`; a missing marker is a fail-closed harness-admission defect. Other harness adapters must likewise obtain one binary-owned admission contract before the model turn.
