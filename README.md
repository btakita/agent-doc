...
- **Managed Codex panes must prove requested capabilities before reuse** — when a Codex document requests network access, required SSH targets, or extra writable roots through `--add-dir`, `start` records a `codex_capability_proof` session-log event after live DNS/SSH/write probes succeed. Route will not blindly reuse an older ready Codex pane without that current proof; it restarts the managed session fresh once with the original launch contract, then reroutes or fails with the missing capability. `agent-doc session status` reports the proof as `proven`, `missing`, or `not_required`.
