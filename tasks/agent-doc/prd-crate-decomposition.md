...
| Template-mode patch parsing, repair, and replay validation | `agent-doc-template` | Active. Pure patch parsing, patchback shape classification, orchestrate patchback contract policy, replay-payload validation, component mutation, boundary repositioning, and duplicate prompt/tail repair live here; orchestration keeps file-backed config/document IO, ops-log, and flow-event adapters. |
...
| `template.rs` / template patchback policy | `agent-doc-template` | Extracted. Pure patch parsing, patchback shape classification, orchestrate patchback contract policy, component mutation, boundary repositioning, and repair helpers live in `agent-doc-template`; file-backed config/document IO, ops-log, and flow-event emission stay in orchestration adapters. |
