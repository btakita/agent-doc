...
- **A retained write must prove its owner before it forbids recovery (`#percellconverge`)** — three sites tell an agent that a write is retained and that recovery is forbidden: `crdt_relay_pending_refusal` (`agent-doc-git-io`), the authority/disk divergence INTERRUPT (`agent-doc-session-check-io`), and `await_editor_replica_no_disk_write` (`agent-doc-document-realtime-io`). "Wait, the intent commits itself, do NOT re-send / force disk / `admin recycle` / `admin reload-lib`" is only true while something durable actually **holds** the write; with no holder, no state edge can fire and the visible edits are stranded, so an agent that obeys loses the work. Observed three times on 2026-08-03 on `tasks/agent-doc/agent-doc-bugs2.md`: `write --commit` printed the deferral while `session-check` (carrying the 0.35.123 ownership fix) called the same state stranded, and `session-check` was right every time. Consolidating the *wording* into one function did not prevent this — the **predicate** is what drifted. So the predicate lives once, in `agent_doc_turn::write_ownership` (`RetainedWriteOwnership::verdict` + `retained_write_remedy`), the I/O shell is `agent_doc_capture_io::retained_write_ownership`, and every site derives its wording from it. A site that cannot prove a holder says stranded and names the recovery (`agent-doc commit <FILE>`, or `write --commit` when a response body is unwritten); the 2026-07-26 do-NOT guidance stays intact for the genuinely-owned case. Never key ownership on editor attachment — `TransportProjection::editor_generation` is a one-way latch (`#editormodelmissing`) and a predicate that trusts it inherits the latch. `agent-doc-turn/tests/retained_write_ownership_guard.rs` enforces both halves: no site re-authors a verdict phrase, and every site actually calls the predicate. This is the interim predicate; per-component ownership ("does anything own the components this turn wrote") replaces its inputs without changing the call sites. Plan: `tasks/agent-doc/plan-per-component-convergence.md`.
...
- **A fresh pane whose admission projection stays unchanged while its trigger remains in the composer is stranded, not idle (`#jbtsiftnosub2`)** — when a freshly-created route pane produces no document cycle, an empty dispatch-ready composer is a legitimate idle no-op (keep it), but a dispatch-ready composer still showing the injected trigger *unsubmitted* is the JB-created-fresh-pane "prompt added but not submitted" drift. Route must resubmit the stranded draft once (bare harness `Enter`) and await the controller-owned turn-admission projection again; keep the session only if the projection advances, otherwise record a `FreshStart` startup miss and fail closed. The classification (`fresh_start_admission_outcome`, `pane_composer_has_pending_trigger`) lives in `agent-doc-controller`; the resubmit consumer lives in `agent-doc-route-io` (`startup.rs`, `startup_ready.rs`). Keep this aligned with `specs/08-session-routing.md`.
...
- **Sync lock acquisition is kernel-blocked and self-releasing** — keep `sync.rs`, `README.md`, `SPEC.md`, and the session/tmux command spec aligned on acquiring `.agent-doc/sync.lock` with a kernel-level `flock(2)` block (no userspace sleep-poll) bounded by the configured wait budget: an uncontended acquire wins on a single non-blocking attempt, and a contended one blocks in the kernel until the holder releases or the budget elapses. `flock(2)` is released automatically when the holding process exits (fd close), so a dead or orphaned owner can never wedge the lock and there is no `/proc` stale-orphan reap. Do not reintroduce a sleep-poll loop or a `/proc`-walking reaper.
...
- **Codex hook backstop is binary-owned** — keep `src/preflight_hook.rs`, `agent-doc-codex-hook-io`, `src/skill.rs`, `SKILL.md`, `runbooks/harness-invocation.md`, `README.md`, and `SPEC.md` aligned on the installed `.codex/hooks.json` / `.codex/config.toml` contract: `UserPromptSubmit` first makes active-document tracking durable, then runs binary preflight for a valid `agent-doc <FILE>` trigger and emits the contract plus success marker as Codex turn context; `Stop` first tries to finish the response cycle deterministically from `last_assistant_message` via the normal repair/write/commit path before falling back to capture-and-block / fail-closed behavior. Missing tracking or preflight proof must never emit the success marker. Empty `last_assistant_message` on an open cycle must still fail closed with diagnostics and tracked-prompt recovery because tool-only/authentication steps (for example MCP OAuth / `authenticate`) are sub-steps, not successful closeout boundaries.
...
src/
  ...
  preflight_hook.rs # Claude/Codex UserPromptSubmit admission; Codex also records active-doc tracking
...
<!-- tsift:code-navigation v=0.1.101 -->
## Code Navigation
