...
mod tests {
    ...
    fn rendered_claude_skill_includes_queue_auto_loop_section() {
        ...
        // #degraded-ipc-no-stall: proven degraded editor transport is not a
        // stop reason, but unproven IPC/direct-write fallback is.
        assert!(
            rendered.contains("#degraded-ipc-no-stall"),
            "auto-loop section must carry the degraded-IPC no-stall rule"
        );
        assert!(
            rendered.contains("file-IPC")
                && rendered.contains("editor/file-IPC path proved delivery")
                && rendered.contains("recovery=retry_without_disk_write")
                && rendered.contains("recovery=direct_write_fallback")
                && rendered.contains("editor_convergence_required")
                && rendered.contains("operator_text_authority_v1"),
            "auto-loop section must distinguish proven file-IPC closeout from unproven/direct-write fallback"
        );
        assert!(
