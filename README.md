...
- **Editor document switches focus before slow sync work** — passive editor sync now resolves the focused markdown file through the controller actor binding and selects its live pane before waiting on the sync lock, prune cleanup, or tmux-router reconciliation, so stale sibling sessions cannot hold tmux focus on the wrong document
...
- **Editor sync lock contention is bounded** — if an older sync process is stuck or orphaned, later selection/layout sync commands wait only for the contention budget, log a warning, and continue to ownership proof / auto-start decisions instead of hanging before the selected file can start
