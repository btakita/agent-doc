...
- **Manual Sync Tmux Layout repairs window order first** — full `agent-doc sync` normalizes the inferred tmux session to `0:agent-doc`, `1:stash`, and adjacent overflow `N:stash` windows before reconciling editor panes; passive `--no-autostart` editor sync keeps that repair step explicit
