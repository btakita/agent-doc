...
- `preflight.queue_continuation_required == true` — the authoritative "there is agent-drainable work the **in-session loop** can take" signal (`#cleardrainsignal`/`#qchurn`). It is `false` when every remaining head is `[clean-session]` (under a live editor-IPC listener) / `[operator-verify]` / inert noise, even while `queue_active == true` and a `queue_trigger` is present. **When it is `false`, do NOT loop and do NOT re-dispatch from this session.** For `[clean-session]` heads this is **not** a dead end (`#cleandrainsup`): the in-session `/loop` defers them because it cannot give them the fresh context the tag asks for, but the supervisor idle-watch force-`/clear`s and re-dispatches each `[clean-session]` head to a freshly-cleared agent — so stopping here hands those heads to the supervisor, it does not abandon them. Only `[operator-verify]`/inert-noise heads are truly nobody's-to-drain. This signal is computed in the binary from the in-session (`DrainScope::Loop`) drainability filter; the supervisor uses the wider `DrainScope::Supervisor` filter (clean-session drainable). Never re-derive drainability by hand from the queue prose. `preflight.queue_drainable_head_count` is the underlying loop-scope count (`0` ⇒ stop the loop).
...
Skip the auto-loop on any failed closeout, `session-check` interruption, or `lint-gate` block — those need explicit operator attention. Skip when `preflight.queue_active == false` (queue drained or halted) **or `preflight.queue_continuation_required == false`** (no loop-drainable head — only `[clean-session]`/`[operator-verify]`/noise heads remain; looping would just churn a no-op `#qchurn` cycle, so report the remaining heads in one line and stop without pausing or punting). When the remaining heads are `[clean-session]`, the supervisor idle-watch drains them on its own (force-`/clear` + re-dispatch per `#cleandrainsup`), so stopping the loop hands them off rather than stalling them — note that in the one-line report. High session-accretion is **not** in that skip list — it is handled by the supervisor `/clear`-and-continue path above, not by stopping.
