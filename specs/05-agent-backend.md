...
- Managed interactive Codex sessions also record a `codex_capability_proof` event after successful live probes when the document requests `codex_network_access: enabled`, `required_ssh_targets`, or extra writable roots via `--add-dir`. Managed non-dispatch route treats a ready actor without a current proof after the latest `session_start` as unproven, restarts the managed Codex session fresh once with the original launch contract, and reroutes only after the fresh session becomes ready. Editor dispatch-only reroutes keep their one-shot live-pane submit boundary and do not turn missing capability proof into a pre-submit restart.
