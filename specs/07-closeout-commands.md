...
- **Full-content IPC source-buffer proof:** Whole-document editor replacements (`fullContent` socket/file IPC) must carry a `patch_id` plus the hash and byte length of the source buffer used to compute the replacement. Editor watchers must reject the replacement without ACK if the live editor/VFS buffer differs from that proof, and the binary must refuse to save a snapshot if a consumed full-content patch leaves anything other than the requested target document.
...
- Concurrent non-component user edits, including markdown or HTML comment tails directly below `agent:exchange`, must remain visible in the working tree/editor buffer and stay outside the assistant closeout commit unless the user explicitly asks to commit that edit.
