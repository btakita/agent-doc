...
- **Codex Stop-hook backstop** — `agent-doc skill install` for Codex now also writes `.codex/hooks.json` and enables `features.codex_hooks = true`, so `UserPromptSubmit` tracks the active document and `Stop` captures `last_assistant_message` into the normal pending/capture ledger before auto-continuing or failing closed on an unfinished cycle
