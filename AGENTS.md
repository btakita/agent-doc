...
- **Realtime operator steering must be surfaced verbatim, never dropped or thrashed (`#realtime-steering-verbatim` / `#no-thrash-steering`)** — a document is realtime: the operator may add a prompt while the turn is active, and every item the operator adds must be addressed and worked on, not committed-and-ignored. When a committed cycle's document carries a fresh operator prompt, the binary must (a) hand the agent that prompt **verbatim** (the full text via `RealtimeSteering::verbatim`, not a first-line preview) and (b) instruct it clearly to address the prompt in the CURRENT turn — never a bare interrupt that reads like a failed closeout. The clear instruction must say: the prior response is already committed in HEAD; re-run `agent-doc <FILE>` to continue; do **not** re-run finalize on the prior response, do **not** `--force-disk` (that clobbers the operator's live edits), and do **not** re-answer a prompt already committed in HEAD (the realtime replica reconciles the committed response back into the live buffer). This is what prevents the thrash loop (repeated preflight/finalize, empty cycles, force-disk clobbers) that a concurrent operator edit used to induce. Keep `session_check.rs`'s `realtime_steering_closeout_guidance`, `baseline_comparison::RealtimeSteering::verbatim`, `prompt_bearing.rs`, `is_committed_prompt_diff_interruption`, `SKILL.md`, and `runbooks/respond.md` aligned; the strategic direction is to back exchange + queue steering with the lazily graph (`SeqCrdt` exchange nodes, `QueueCell` steering) so concurrent editing reconciles as a CRDT instead of a bespoke merge.
