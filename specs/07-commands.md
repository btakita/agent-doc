...
- Completed-backlog reap guard: after a committed cycle, `session-check` fails closed if the live document still contains any `- [x]` backlog items. Successful preflight/closeout must either reap those items through both the working tree and snapshot or stop with an explicit failure instead of silently carrying completed items forward.
