...
- **`repair` closes git-backed recovery immediately** — direct `agent-doc repair` / `recover` now runs the normal commit boundary after replaying or deduping a pending response in git, instead of leaving repaired assistant content uncommitted until a later `preflight`
