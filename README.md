...
- **Daily GC covers stale operational diagnostics** — `agent-doc gc` now prunes old Codex blocked-stop diagnostics with the same seven-day retention as repair-blocked records and closes stale `starting` controller actors when no fresh supervisor lease still proves that generation is in progress, then re-emits `session-actors.json` from SQLite.
